<template>
  <div class="app-container manage">
    ...
    <div class="sheet">
      ...
      <div class="search-bgc search-mr">
        <el-form :model="form" class="form-data">
          <el-form-item label="员工姓名" label-width="80px">
            <el-input v-model.trim="userName" size="mini" class="wh-100" maxlength="25"></el-input>
          </el-form-item>
          ...
          <el-form-item label="手机号码" label-width="80px">
            <el-input v-model.trim="telephone" size="mini" maxlength="11" class="wh-120" @input="telephone = $limitPhone2(telephone)"></el-input>
          </el-form-item>
          ...
          <el-form-item label="所属角色" label-width="80px">
            <el-select v-model="form.addRoleIdList" placeholder="请选择" size="mini" class="wh-120">
          ...
          <el-button type="primary" size="mini" icon="el-icon-search" class="seek el-button-primary" @click="searchData">查询</el-button>
          <el-button type="primary" size="mini" class="add-staff el-button-primary" @click="addStaff" v-has="'org:user:add'">新增员工</el-button>
        </el-form>
      </div>
      <div class="table-data" style="height:  86%;">
        <el-table :data="tableData" ref="tables" stripe v-loading="loading" height="98%" :header-cell-style="{ background: '#f1f6ff' }" :tooltip-effect="'light'">
          ...
          <el-table-column prop="areaName" label="所管工区" align="center" v-if="orgType == 5" show-overflow-tooltip> </el-table-column>
          <el-table-column label="操作" align="center" width="300">
            <template slot-scope="{ row }">
              <el-button plain size="mini" type="primary" @click="lookOver(row)" v-has="'org:user:query'">查看</el-button>
              <el-button plain size="mini" type="warning" @click="editStaff(row)" v-has="'org:user:edit'">编辑</el-button>
              <el-button plain size="mini" type="danger" @click="handleDelete(row)" v-if="row.enableStatus" v-has="'org:user:editStatus'">禁用</el-button>
              <el-button plain size="mini" type="success" @click="handleDelete(row)" v-has="'org:user:editStatus'" v-else>启用</el-button>
    ...
    <!-- 弹窗 -->
    <el-dialog :close-on-press-escape="false" :title="title" width="45%" top="5vh" :visible.sync="popup" v-dialogDrag :close-on-click-modal="false" v-loading="loading">
      <el-form :model="form" class="sech-form" ref="formInline">
        <el-row style="width:100%">
          <el-col :span="5">
            <el-form-item label="姓名" label-width="40px">
              <el-input v-model.trim="form.userName" size="mini" :disabled="typeStatus" class="wh-100" maxlength="25"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="4">
            <el-form-item label="性别" label-width="40px">
              <el-select v-model="form.sex" placeholder="请选择" :disabled="typeStatus" size="mini" style="width:100%">
                <el-option label="男" :value="1"></el-option>
                <el-option label="女" :value="2"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :span="6">
            <el-form-item label="手机号码" label-width="70px">
              ...
              <el-input v-model="form.telephone" size="mini" :disabled="typeStatus || !!form.realNameVerify" maxlength="11" @input="telephoneInput"></el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="5">
            <el-form-item label="验证码 " label-width="60px" v-if="codeStatus">
              <el-input v-model="form.code" size="mini" class="wh-100" maxlength="4" @input="form.code = $limitPhone2(form.code)"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="5">
          ...
          <el-col :span="5">
            <el-form-item label="备注" label-width="50px">
              <el-input v-model="form.remark" size="mini" :disabled="typeStatus" maxlength="100"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="7" v-if="[2, 3, 9, 4, 6, 7].includes(this.orgType)">
            <el-form-item :label="[6, 7, 4, 9, 2, 3].includes(orgType) ? '管理项目' : '管理单位'" label-width="80px">
              <el-select v-model="form.projectIdListSet" multiple :disabled="typeStatus" collapse-tags placeholder="请选择" size="mini">
                <el-option v-for="item in orgNameList" :key="item.pkId" :label="item.projectName" :value="item.pkId"> </el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <!-- v-if="orgType == 3 || orgType == 11" -->
          <el-col :span="7" v-else-if="orgType != 5 && orgType != 0">
            <el-form-item :label="[6, 7, 4, 9, 2, 3].includes(orgType) ? '管理项目' : '管理单位'" label-width="80px" v-if="orgType != 5 && orgType != 0">
              <el-select v-model="form.manageCustomIdList" multiple :disabled="typeStatus" collapse-tags placeholder="请选择" size="mini">
                <el-option v-for="item in orgNameList" :key="item.pkId" :label="item.orgName" :value="item.pkId"> </el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
      ...
      <el-tabs v-model="tabType">
        ...
        <el-tab-pane label="数据权限" name="3" v-if="![0].includes(orgType) && orgType != 10 && orgType != 12 && orgType != 11"></el-tab-pane>
      </el-tabs>
      <!-- 权限表格 -->
      <div class="permIssion-box" style="margin-top: 5px;position: relative;">
        ...
        <dataPermisson ref="dataPermisson" :manageAuthorize="manageAuthorize" :treeData="treeData3" :treeCheckList="selectTree3" disabled v-if="popup" v-show="tabType == 3"></dataPermisson>
        <!-- <div class="data-authority dialog-form" v-show="tabType == 3">
        <p class="authority-title">物资模块：</p>
        <el-radio-group v-model="form.dataCode" class="authority-box" :disabled="typeStatus">
          <el-radio class="authority-item" label="0">无单价和金额权限</el-radio><br />
          <el-radio class="authority-item" label="1">可查看单价和金额权限</el-radio><br />
          <el-radio class="authority-item" label="2">可编辑单价和金额权限</el-radio>
        </el-radio-group>
      </div> -->
      </div>
      ...
      <span slot="footer" class="dialog-footer" v-if="!typeStatus">
        ...
        <el-button size="mini" type="primary" class="el-button-primary" @click="getchekcData">保 存</el-button>
      </span>
    </el-dialog>
    <el-dialog :visible.sync="verify" center top="30vh" title="请完成下列验证!" width="24%" @close="closePop" :close-on-click-modal="false">
      <!-- <div class="dialog-form">
        <img :src="imgCode" class="spacing" />
        <el-input v-model="identifyingCode" placeholder="请输入图形验证码"></el-input>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" class="el-button-primary" @click="getImgecode">切换验证码</el-button>
...
export default {
  ...
  mounted() {
    ...
    if ([2, 3, 9, 4, 6, 7].includes(this.orgType)) {
      this.getItemList();
    } else {
      this.subsetOrgList();
    }
    // if(this.orgType ==3 || this.orgType ==11){
  ...
  methods: {
    ...
    telephoneInput(val) {
      ...
      if (value.length == 11) {
        ...
        this.$api.availableTelephone({ telephone: value, sourceType: 2 }).then(res => {
          if (res.data) {
    ...
    editStaff(item) {
      ...
      // this.$api.byTelephone({ telephone: item.telephone, sourceType: 2 }).then(res => {
      //   if (res.data.length) {
      //     this.codeStatus = true;
      //   } else {
      //     this.codeStatus = false;
      ...
      this.$api
        ...
        .then(res => {
          if (res.code === 200) {
            ...
            this.form.projectIdListSet = [2, 3, 9, 4, 6, 7].includes(this.orgType) ? res.data.projectIdListSet : [];
            this.form.manageCustomIdList = ![2, 3, 9, 4, 6, 7].includes(this.orgType) ? res.data.manageCustomIdList : [];
            //
            this.form.realNameVerify = res.data.realNameVerify;
            this.form.dataCode = res.data.dataCode;
            this.roleListdata = res.data.roleList; // 原来拥有的角色，用来判断增加删除分类处理
            this.popup = true;
            this.$nextTick(() => {
              this.selectTree3 = res.data.dataAuthorizeVo && res.data.dataAuthorizeVo.viewAuthorize ? res.data.dataAuthorizeVo.viewAuthorize : [];
              this.manageAuthorize = res.data.dataAuthorizeVo && res.data.dataAuthorizeVo.manageAuthorize ? res.data.dataAuthorizeVo.manageAuthorize : { type: "", userId: [] };
            });
          } else {
            this.$message.warning(res.msg);
          }
    ...
    getchekcData() {
      ...
      if (this.form.userName.length == 0) {
        return this.$message.warning("请输入员工姓名");
      }
      ...
      if (this.codeStatus && !this.form.uuid) {
    ...
    // 获取角色
    getRoles() {
      this.$api.getOrgAllRoleList().then(res => {
        if (res.code == 200) {
      ...
      for (let i = 0; i < arr.length; i++) {
        for (let j = 0; j < roleArr.length; j++) {
          if (arr[i] == roleArr[j].pkId) {
            ...
            console.log(roleArr[j]);
            this.selectTree = roleSelect;
          }
        }
      }
      if (![0, 6, 7].includes(this.orgType)) {
