<template>
  <div>
      ...
      <div class="dialog-forms">
        <div class="workflowDialog">
          ...
          <div :class="{ 'right-content': [7, 8, 10].includes(dialogType) }" :style="{ width: [7, 8, 10].includes(dialogType) ? '' : '100%' }">
                  ...
                  <div class="approList">
                    ...
                    <div class="approListTable table-data">
                      <el-table :tooltip-effect="'light'" stripe style="width: 100%" height="100%" :data="signDetail.sealApproveDetailVo.approveDetailList" :header-cell-style="{ background: '#f1f6ff' }">
                        ...
                        <el-table-column label="审批意见" align="center" prop="approveComment"></el-table-column>
            ...
            <el-container v-show="nowTab === 1" v-if="dialogType != 10">
              <!-- 左侧流程图 -->
              ...
              <el-main style="position: relative" :class="[{ h600: dialogType === 4, h700: dialogType === 5 || dialogType === 6 }]">
                ...
                <el-tabs v-model="activeName1">
                      ...
                      <el-row :span="24" :key="flowShow">
                        ...
                        <el-col :span="8">
                          <div class="dialog-header-item">
                            ...
                            <div class="value">
                              <el-date-picker size="mini" class="mr5" :class="dialogType === 1 && flowShow ? 'wh-140' : 'wh-120'" value-format="yyyy-MM-dd" v-model="formData.beginDate" disabled></el-date-picker>
                        ...
                        <el-col :span="8">
                          <div class="dialog-header-item" v-if="pricePermission">
                        ...
                        <el-col :span="8" v-if="[1, 2, 3].includes(dialogType) && addNum == 1">
                          <div class="dialog-header-item">
                            ...
                            <div class="value">
                              <!-- <el-input size="mini" :value="roleName" disabled></el-input> -->
                      ...
                      <el-tabs v-model="activeName2" :key="cardRandomKey">
                        <el-tab-pane label="分项列表" name="first" v-if="dialogType < 4 && addNum == 1">
                          <div class="table1 table-data">
                            <!-- :show-summary="activeName2 === 'first'&&treeStatsItem.length"  :summary-method="getSummaries" -->
                            <el-table :tooltip-effect="'light'" height="100%" :header-cell-style="{ background: '#f1f6ff' }" style="width: 100%" :data="treeStatsItem" :tree-props="{ children: 'children' }" row-key="pkId" ref="tables1" :key="cardRandomKey">
                              <!-- <el-table-column align="center" label="选择" width="50" v-if="dialogType == 1 || dialogType == 2 || dialogType == 3">
                        <template slot-scope="{ row }">
                          <el-checkbox :disabled="row.isTemporary !== 1 || dialogType == 2 || dialogType == 3" v-if="row.grade == 3" v-model="checkList" @change="selectRow($event, row)" :label="row.fkItemId"><span></span></el-checkbox>
                        </template>
                      </el-table-column> -->
                              ...
                              <!-- <el-table-column align="center" label="结算单价" v-if="pricePermission">
                                <template slot-scope="{ row }">
                                  <el-input size="mini" v-if="dialogType === 1 && row.grade === 4" v-model.trim="row.settlePrice" @keydown.native="keyDown" @input="input($event, 2, row)" type="number" class="price"></el-input>
                                  <p v-else-if="dialogType !== 1 && row.grade === 4">
                                    {{ row.settlePrice }}
                                  </p>
                                </template>
                              </el-table-column> -->
                        ...
                        <el-tab-pane label="费用列表" name="second" v-if="dialogType < 4 && addNum == 1">
                          <div class="table1 table-data">
                            <!--  :show-summary="activeName2 === 'second'&&engExpenseDetail.length"  :summary-method="getSummaries3" -->
                            <el-table :tooltip-effect="'light'" stripe :header-cell-style="{ background: '#f1f6ff' }" height="100%" style="width: 100%" :data="engExpenseDetail">
                              ...
                              <el-table-column align="center" label="结算金额" v-if="pricePermission" prop="settleAmount">
                                ...
                                @blur="row.settleAmount = $event.target.value"
                                @input="secondInput($event, 1, row, row.costAmount)"
                                :maxlength="secondMaxLength" -->
                                <template slot-scope="{ row }">
                                  <!-- @blur="expenBlur(row)" -->
                                  <el-input v-model="row.settleAmount" v-if="dialogType === 1 && row.type == 1" size="mini" @input="secondInput($event, 1, row, row.costAmount)"></el-input>
                        ...
                        <el-tab-pane label="分项扣减列表" name="thith" v-if="dialogType < 4 && addNum == 1 && treeItemDeductions.length">
                          <div class="table1 table-data">
                            <!--  :show-summary="activeName2 === 'thith'&&!!treeItemDeductions.length" :summary-method="getSummaries" -->
                        ...
                        <!-- <el-tab-pane label="结算表" name="forth" v-if="dialogType < 4">
                          <div class="table1 table-data">
                            <el-table :tooltip-effect="'light'" stripe :header-cell-style="{ background: '#f1f6ff' }" height="100%" style="width: 100%" :data="detail" :show-summary="activeName2 === 'forth'" :summary-method="getSummaries">
                        ...
                        <el-tab-pane label="汇总表" name="forth" v-if="dialogType > 3 || (dialogType < 4 && addNum == 2)">
                          <div class="table1 table-data">
                            <el-table :tooltip-effect="'light'" stripe :header-cell-style="{ background: '#f1f6ff' }" height="100%" style="width: 100%" :data="formData.settleSummaryList" :show-summary="activeName2 === 'forth'" :summary-method="getSummaries2">
                              <!-- <el-table-column align="center" type="index" label="序号" width="60"></el-table-column> -->
                        ...
                        <!-- 只在审批开始显示 -->
                        <el-tab-pane :label="item.chapterName" :name="index + ''" v-for="(item, index) in settleChapterVos" :key="item.pkId" v-if="dialogType > 3 || addNum == 2">
                          <div class="table1 table-data">
                        ...
                        <el-tab-pane label="验收流程附件" name="files1" v-if="(dialogType > 3 && treeReProcessFile.length) || dialogType < 4">
                          <div class="table1 table-data">
                            <el-table :tooltip-effect="'light'" :header-cell-style="{ background: '#f1f6ff' }" :data="treeReProcessFile" height="100%" style="width: 100%" :tree-props="{ children: 'children' }" row-key="pkId" ref="tables3">
                              ...
                              <el-table-column label="上传时间" align="center" prop="createTime">
                                <template slot-scope="{ row }">
                                  <span v-if="row.grade === 4">{{ row.createTime }}</span>
                                </template>
                              </el-table-column>
                        ...
                        <el-tab-pane :label="dialogType === 1 && addNum == 1 ? '上传附件' : '附件'" name="files2">
                          <div class="table1 table-data">
                            <!-- 用单独一个table使用v-if隐藏某些行会导致样式错乱，所以使用2个table -->
                            <el-table :tooltip-effect="'light'" stripe :header-cell-style="{ background: '#f1f6ff' }" :data="accessoryList" height="100%" style="width: 100%" v-if="dialogType <= 3 && addNum == 1">
                              <el-table-column label="序号" type="index" width="80" align="center"> </el-table-column>
                            ...
                            <el-table :tooltip-effect="'light'" stripe :header-cell-style="{ background: '#f1f6ff' }" :data="accessoryList" height="100%" style="width: 100%" v-if="dialogType > 3 || addNum == 2">
                              <el-table-column label="序号" type="index" width="80" align="center"> </el-table-column>
                  ...
                  <el-tab-pane label="审批记录" name="second" v-if="[4, 5, 6, 7, 8].includes(dialogType)">
    ...
    <approveCodeDialog ref="approveCodeDialog" :approveCodeDialog.sync="approveCodeDialog" :sign-date="signDate" :signed="signed" @approveOk="approveOk" @close="loading = false" :biz-type="1" :siStatus="siStatus" :noApprove="[9, 10].includes(dialogType) ? true : false" @signature="signature" :appCodeType="appCodeType" @getNew="getNew"/>
...
export default {
  ...
  methods: {
    ...
    getSettleWorkflowTable() {
      this.$api.getSettleWorkflowTable({ settleType: 1 }).then(res => {
        if (res.code == 200) {
          ...
          if (res.data) {
            res.data.workflowNodeDTOS.forEach((item, index) => {
              if (item.nodeType == 2) {
                ...
                console.log(item.prodSysRoleVo.selectedUserId);
              }
            });
    ...
    approvalSet() {
      ...
      if (this.approvalData.launchType == 2 && !nodeArr[0].prodSysRoleVo.selectedUserId) {
        // return this.$message.warning("首个节点必须有人");
        return this.$message.warning("此流程只能由【质检工程师】发起");
      }
      let arr = this.approvalData.workflowNodeDTOS;
    ...
    approvalCancel(num) {
      ...
      if (this.dialogType === 1 && !this.formData.pkId) {
        // this.approvalStatus = false;
        this.getApproverSettle({ settleType: 1 });
      }
      if (this.dialogType === 1 && this.formData.pkId) {
        // this.approvalStatus = true;
        this.getApproverSettle({ settleType: 1, pkId: this.formData.pkId });
      }
    },
    dataExport() {
      this.loading = true;
    ...
    urlPush(url, type) {
      let width = this.pdfTitle[this.pdfUrl.length].type == "2" ? 842 : 596;
      let height = this.pdfTitle[this.pdfUrl.length].type == "2" ? 596 : 842;
      this.pdfUrl.push({ path: url, width: width, height: height, name: this.pdfTitle[this.pdfUrl.length].title, direction: this.pdfTitle[this.pdfUrl.length].type });
    ...
    dataPrinting() {
      ...
      if (this.pdfTable.length == 0) {
        ...
        this.pdfTable.forEach((item, idx) => {
          htmlToPdf
            .getPdf("#pdfDom" + idx, this.pdfTitle[idx].title, false, this.pdfTitle[idx].type) //download：false为不下载，这里调用了刚刚引用的全局函数，.then得到的值是base64位的pdf文件
            .then(res => {
              // console.log(res)
              const _this = this;
    ...
    getCustomListByType(isTrue) {
      api.getCustomListByType({ contractType: 1 }).then(res => {
        if (res.code === 200) {
          this.contractList = res.data;
          if (isTrue) {
            this.formData.taxRate = res.data.filter(item => item.pkId == this.formData.fkContractId)[0].taxRate;
      ...
      api
        ...
        .then(res => {
          ...
          if (res.code === 200) {
            ...
            // this.statsItem = res.data.settleDetails;dialogType showNext
            if (type === 2) {
            ...
            let fileArr = [];
            let arr2 = res.data.itemReProcessFileVos ? res.data.itemReProcessFileVos.filter(item => item.grade !== 4) : [];
            arr2.forEach(item => {
              fileArr.push(item);
        ...
        .then(res => {
          ...
          if (res.code === 200) {
            if (res.data) {
              ...
              this.reProcessFile = fileArr;
              this.treeReProcessFile = this.listToTree(this.reProcessFile, "0");
              this.cardRandomKey = Math.random();
              this.activeName2 = "forth";
              this.tableDTOS = res.data.workflowTableList ? res.data.workflowTableList : [];
              console.log(222, this.tableDTOS);
              ...
              if (this.tableDTOS) {
                ...
                // return console.log(this.tableDTOS)
                this.tableDTOS.forEach(item => {
                  item.tableHtml = this.utils.unzip(item.tableHtml);
                  if (item.tableHtml instanceof Array) {
                  } else {
    ...
    // 获取分包单位
    getSubList() {
      api.searchCustomByProOrgId().then(res => {
        if (res.code === 200) {
      ...
      api
        ...
        .then(res => {
          if (res.code === 200) {
            ...
            let fileArr = [];
            let arr5 = res.data ? res.data.filter(item => item.grade !== 4).map(item => ({ ...item })) : [];
            arr5.forEach(item => {
              fileArr.push(item);
              if (item.grade === 3 && item.reProcessFileVos) {
                fileArr.push(...item.reProcessFileVos.map((item2, index2) => ({ ...item2, grade: 4, pkId: item.fkItemId + "" + index2, fkParentItemId: item.fkItemId })));
      ...
      api
        ...
        .then(res => {
          ...
          if (res.code === 200) {
            ...
            this.treeReProcessFile = this.treeReProcessFile.filter(item => item.hasFile);
            console.log("notod", this.treeReProcessFile);
            // let fileArr1=arr.filter(item=>item.grade!==4)
          } else {
            this.$message.warning(res.msg);
          }
    ...
    btnOk(settleStatus, auditStatus, unique) {
      if (this.dialogType === 1 || this.dialogType === 3) {
        const { endDate, settleDate, settleName } = this.formData;
        if (!endDate) {
          return this.$message.warning("结算周期结束日期不能为空");
        }
        // if (!fkApproverId) {
        ...
        if (!this.formData.settleDetails.length && !this.formData.engExpenseDetailAdds.length && !this.formData.itemDeductionAdds.length) {
        ...
        // this.formData.engDeductionDetailAdds = this.itemDeductions;
        ...
        // };
        this.formData.beginDate = /\d{4}-\d{1,2}-\d{1,2}/g.exec(this.formData.beginDate);
        this.formData.beginDate = this.formData.beginDate.toString();
        ...
        let data = {
          ...
          settleDetails: this.statsItem,
          engExpenseDetailAdds: this.formData.engExpenseDetailAdds,
          fileList: this.accessoryList.filter(item => item.fileUrl),
          prodWorkflowUserAdds: prodWorkflowUserAdds,
    ...
    projectExamine(unique) {
      if ([5, 6].includes(this.dialogType)) {
        ...
        return this.accountability(tableData, unique);
        if (num1 > 0) {
          // console.log(this.pdfTitle)
    ...
    accountability(tableData, unique) {
      const data = { ...this.approval, fileList: [...this.accessoryList, ...this.appAccList.filter(item => item.fileUrl)], workflowTableList: tableData, unique };
      this.loading = true;
      api
        .projectExamine(data)
        .then(res => {
          if (res.code === 200) {
            ...
            this.$message.warning(res.msg);
            if (data.tableDTOS.length != 0) {
              let arr = [];
              data.tableDTOS.forEach(item => {
    ...
    contractChange(e) {
      ...
      this.getBeforeAddSetting({ customId: "", contractId: this.formData.fkContractId });
    },
    keyDown(e) {
      const key = e.key;
      if (key === "e" || key === "-") {
    ...
    input(e, type, item) {
      ...
      if (type === 1) {
        ...
        // let obj1 = this.statsItem.filter(
        //   item1 => item1.fkItemId === item.fkParentItemId
        // )[0];
        // let obj2 = this.statsItem.filter(
        //   item1 => item1.fkItemId === obj1.fkParentItemId
        // )[0];
        // let obj3 = this.statsItem.filter(
        //   item1 => item1.fkItemId === obj2.fkParentItemId
        // )[0];
        ...
        //   });
        // obj1.finishAmount = sum1;
        // this.statsItem
        //   .filter(item1 => item1.fkParentItemId === obj2.fkItemId)
        //   .forEach(item1 => {
        //     sum2 += item1.finishAmount - 0;
        //   });
        // obj2.finishAmount = sum2;
        // this.statsItem
        //   .filter(item1 => item1.fkParentItemId === obj3.fkItemId)
        //   .forEach(item1 => {
        //     sum3 += item1.finishAmount - 0;
        //   });
        // obj3.finishAmount = sum3;
      }
    },
    endTimeChange(val) {
  ...
  computed: {
    ...
    dialogTitle() {
      if (this.dialogType === 1 && !this.formData.pkId) return "新增结算";
      if (this.dialogType === 1 && this.formData.pkId) return "编辑结算";
    ...
    detail() {
      ...
      // let arr3 = this.engExpenseDetail.map(item => ({ ...item, itemName: item.detailName, price: item.costAmount, settlePrice: item.costAmount, designNumber: 1, designAmount: item.costAmount, finishNumber: item.costAmount ? (item.settleAmount / item.costAmount).toFixed(2) - 0 : 0, finishAmount: item.settleAmount, settleAmount: item.settleAmount }));
      ...
      //     if (filterArr[i].fkItemDetailId === arr4[j].fkItemDetailId) {
      //       // arr4[j].finishAmount = (arr4[j].finishAmount - 0 + (filterArr[i].finishAmount - 0)).toFixed(2) - 0;
      //       arr4[j].designNumber = (arr4[j].designNumber - 0 + (filterArr[i].designNumber - 0)).toFixed(2) - 0;
      //       arr4[j].designAmount = (arr4[j].designAmount - 0 + (filterArr[i].designAmount - 0)).toFixed(2) - 0;
      //       arr4[j].finishNumber = (arr4[j].finishNumber - 0 + (filterArr[i].finishNumber - 0)).toFixed(2) - 0;
      //       arr4[j].settleAmount = (arr4[j].settleAmount - 0 - (filterArr[i].settleAmount - 0)).toFixed(2) - 0;
      //       flag = false;
      //     }
      //   }
      //   if (flag) {
      //     arr4.push({ ...filterArr[i], settleAmount: 0 - filterArr[i].settleAmount });
      //   }
      // }
      // console.log(arr4, filterArr);
      // let detailList = [...arr2, ...arr3, ...arr4.map(item => ({ ...item, finishAmount: item.settleAmount, finishNumber: 0 - item.finishNumber }))];
  ...
  watch: {
    ...
    //       this.isDetail = 1;
    //     }
    //     if (!this.statsItem.length || !this.detail.length) {
    //       this.getStatsItemFinish();
    //     }
    //   }
    // },
    ...
    detail: {
      ...
      handler(val) {
        if (this.dialogVisible && this.dialogType < 4) {
          let values = val.map(item => (isNaN(item.currentSettleAmount) ? 0 : Number(item.currentSettleAmount)));
          let sum = 0;
          if (!values.every(value => isNaN(value))) {
    ...
    dialogVisible(val) {
      if (val) {
        ...
        if (this.dialogType === 8) {
          ...
          this.statsItem = this.signData.settleDetails;
          this.treeStatsItem = this.listToTree(this.statsItem, "0");
          this.approverList = this.signData.approveDetailList;
          this.approval.currentNodeId = this.signData.currentNodeId;
          ...
          let fileArr = [];
          let arr2 = this.signData.itemReProcessFileVos ? this.signData.itemReProcessFileVos.filter(item => item.grade !== 4) : [];
          arr2.forEach(item => {
            fileArr.push(item);
            if (item.grade === 3 && item.reProcessFileVos) {
              fileArr.push(...item.reProcessFileVos.map((item2, index2) => ({ ...item2, grade: 4, pkId: item.fkItemId + "" + index2, fkParentItemId: item.fkItemId })));
          ...
          this.treeReProcessFile = this.listToTree(this.reProcessFile, "0");
          this.cardRandomKey = Math.random();
          this.activeName2 = this.treeStatsItem.length ? "first" : this.engExpenseDetail.length ? "second" : this.treeItemDeductions.length ? "thith" : "forth";
          ...
          if (this.pkId) {
            if (this.dialogType === 1 || this.dialogType === 2 || this.dialogType === 3) {
              let type = this.dialogType === 2 ? 1 : 2;
              if (this.again) {
                type == 2;
              }
              this.getProjectSettingById(type);
            } else {
              let type = this.dialogType === 4 ? 1 : 2;
        ...
        if ([1, 2].includes(this.dialogType)) {
          if (this.pkId) {
            // this.approvalStatus = false;
            this.getApproverSettle({ settleType: 1, pkId: this.pkId });
          } else {
            this.approvalStatus = false;
            this.getApproverSettle({ settleType: 1 });
          }
