  ...
  <div class="app-container">
    ...
    <div class="search-bgc search-mr">
      <el-form :model="searchData" class="form-data search-bar-box">
        ...
        <el-button size="mini"  v-if="typeShow" type="primary" class="seek" @click="queryData" icon="el-icon-search">查询</el-button>
        <el-button type="primary"  v-if="typeShow" size="mini" class="add-staff" @click="addRecord" v-has="'security:inspect:add'">新增记录</el-button>
      </el-form>
    </div>
    <div class="tab-container table-data"  v-if="typeShow">
      <el-table :tooltip-effect="'light'" :data="tableData" v-loading="loading" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }">
        ...
        <el-table-column prop="inspectName" label="检查主题" align="center" show-overflow-tooltip> </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="400">
          <template slot-scope="{ row }">
            <el-button plain size="mini" type="primary" @click="lookOver(row)" v-has="'security:inspect:query'">查看</el-button>
            <el-button plain size="mini" v-has="'security:inspect:update'" type="success" @click="editRecord(row)" v-if="row.editStatus">编辑</el-button>
            <el-button plain size="mini" @click="handleDownload(row.enclosures)" v-if="row.enclosures.length" v-has="'security:inspect:download'">下载</el-button>
            <el-button plain size="mini" v-has="'security:inspect:delete'" type="danger" @click="deleteRecord(row.pkId)" v-if="row.deleteStatus">删除</el-button>
          </template>
        </el-table-column>
      ...
      <div class="dialog-form">
        <el-form :model="form" :rules="rules" ref="form">
          ...
          <el-form-item label="检查主题" prop="inspectName" label-width="110px">
            <el-input size="mini" v-model="form.inspectName" :disabled="disabledStatus" placeholder="请输入检查主题"></el-input>
          </el-form-item>
          ...
          <el-form-item label="附件" label-width="110px" prop="enclosures">
            <div>
              <el-button size="small" v-if="!disabledStatus && form.enclosures.length != 10" type="primary" @click="uploading" :disabled="disabledStatus">点击上传</el-button>
              <div class="upLoad-show">
                <p v-for="(item, index) in form.enclosures" :key="index">
                  ...
                  <span v-if="!disabledStatus">
                    <el-button size="mini" style="color: red;border: none;" icon="el-icon-close" @click="remove(item, index)"></el-button>
                  </span>
                </p>
              </div>
            </div>
