<template>
  <div>
    <!-- <search-bar class="search-bar-demo" @search="search" :options="searchOptions" :total="totalCount" labelWidth="8em" ref="searchBar" @change="change" :showHandler="false"></search-bar> -->
    <div class="search-bar-box">
      <el-form :model="listQuery" class="form-data">
        <el-form-item label="工程类型" label-width="80px">
          <span>{{ headDate.enginerringTypeName }}</span>
        </el-form-item>
        <el-form-item label="专业类别" label-width="80px">
          <el-select v-model="listQuery.fkProfessionalTypeId" size="mini" class="wh-100">
            <el-option label="全部" value=""></el-option>
            <el-option v-for="item in professionalType" :key="item.pkId" :label="item.engTypeName" :value="item.pkId"> </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="资料类型" label-width="80px">
          <el-select v-model="listQuery.bookType" size="mini" class="wh-100">
            <el-option label="全部" value=""></el-option>
            <el-option label="技术规范" value="0"></el-option>
            <el-option label="安全规范" value="1"></el-option>
            <el-option label="验收标准" value="2"></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="规范名称" label-width="90px">
          <el-input v-model="listQuery.bookName" size="mini" class="wh-100" maxlength="50"></el-input>
        </el-form-item>
        <el-button size="mini" type="primary"  icon="el-icon-search" class="seek ml-20 el-button-primary" @click="search">查询</el-button>
        <el-button type="primary" v-has="'customized:customData:add'" class="btn-right el-button-primary" size="mini" @click="addUnit">上传</el-button>
      </el-form>
    </div>

    <div class="tab-container">
      <div class="table-data" style="height:87%">
      <el-table ref="tables" stripe :data="tableData" plain style="width: 100%" v-loading="loading" height="88%" :header-cell-style="{ background: '#f1f6ff' }" :cell-style="{ align: 'center' }" :tooltip-effect="'light'">
        ...
        <el-table-column align="center" prop="bookType" label="资料类型" min-width="120">
          <template slot-scope="{ row }">
            <span v-if="row.bookType == '0'">技术规范</span>
            <span v-if="row.bookType == '1'">安全规范</span>
            <span v-if="row.bookType == '2'">验收标准</span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="bookName" label="规范名称" min-width="180"></el-table-column>
        <el-table-column align="center" prop="bookPages" label="页数" min-width="60" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column align="center" prop="alarmLevel" label="操作" fixed="right" min-width="320">
          <template slot-scope="scope">
            <el-button size="mini" plain type="primary" v-has="'customized:customData:query'" @click.native="preview(scope.row)">预览</el-button>
            <el-button type="success" plain size="mini" v-has="'customized:customData:download'" @click.native="download(scope.row)">下载</el-button>
            <el-button size="mini" plain v-has="'customized:customData:update'" @click.native="edit(scope.row)">编辑</el-button>
            <el-button type="danger" plain size="mini" v-has="'customized:customData:delete'" @click.native="remove(scope.row)">删除</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="addTitle" width="520px" :visible.sync="show1" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <el-form :model="form" ref="form" :rules="rules" class="dialog-form">
        <el-form-item label="工程大类" prop="fkEnginerringTypeId" :label-width="formLabelWidth">
          <el-select size="mini" style="width: 100%" placeholder="请选择" v-model="form.fkEnginerringTypeId" disabled>
        ...
        <el-form-item label="资料类型" prop="bookType" :label-width="formLabelWidth">
          <el-select size="mini" style="width: 100%" placeholder="请选择" v-model="form.bookType">
            <el-option label="技术规范" value="0"> </el-option>
            <el-option label="安全规范" value="1"> </el-option>
            <el-option label="验收标准" value="2"> </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="规范名称" prop="bookName" :label-width="formLabelWidth">
          <el-input size="mini" v-model="form.bookName" autocomplete="off" maxlength="50"></el-input>
        </el-form-item>
        <el-form-item label="备注" prop="remark" :label-width="formLabelWidth">
          <el-input size="mini" v-model="form.remark" type="textarea" style="resize: none" :rows="3" maxlength="100" placeholder="请输入内容" autocomplete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
        ...
        if (valid) {
          this.projectType.forEach(items => {
            if (items.pkId == this.form.fkEnginerringTypeId) {
              ...
              this.form.fkOrgId = this.headDate.pkId;
    ...
    search(form) {
      ...
      // this.listQuery.bookType = form.bookType
    ...
    conditionPdf() {
      ...
      if (data.bookType.length == 0) {
        delete data.bookType;
      }
      if (data.fkEnginerringTypeId.length == 0) {
        delete data.fkEnginerringTypeId;
      }
      if (data.bookName.length == 0) {
        delete data.bookName;
      }
      if (data.fkProfessionalTypeId.length == 0) {
        delete data.fkProfessionalTypeId;
      }
      this.$api.conditionPdf(data).then(res => {
        if (res.code == 200) {
    ...
    edit(row) {
      ...
      this.form.fkProfessionalTypeId = this.form.fkProfessionalTypeId + "";
      this.form.bookType = this.form.bookType + "";
    },
    ...
    addUnit() {
      ...
      this.form = {
        bookName: null, //书名
        fkEnginerringTypeId: this.headDate.fkEnginerringTypeId + "", //工程类型
        fkProfessionalTypeId: "",
        remark: null, //备注
        bookUrl: null, //资料地址
        bookPages: null //pdf页数
