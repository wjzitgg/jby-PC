<template>
  <div>
    <!-- <search-bar class="search-bar-demo" @search="search" :options="searchOptions" :total="totalCount" labelWidth="8em"
      ref="searchBar" :showHandler="false" @change="change"></search-bar> -->
    <!-- <el-button type="primary" size="mini" class="search-mr" @click="addUnit" v-has="'system:form:add'">新建表格</el-button> -->
    <div class="search-bar-box">
      <el-form :model="searchData" class="form-data">
        <el-form-item label="工程大类:" label-width="80px" v-if="headDate.orgType == 5">
          <span>{{ headDate.enginerringTypeName }}</span>
        </el-form-item>
        <el-form-item label="专业类别" label-width="80px" v-if="headDate.orgType == 5">
          <el-select v-model="searchData.fkProfessionalTypeId" size="mini" class="wh-200">
            <el-option label="全部" value=""></el-option>
            <el-option v-for="(item, idx) in itemType" :key="idx" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="表格名称" label-width="90px">
          <el-input v-model="searchData.tableName" size="mini" class="wh-200" maxlength="50"></el-input>
        </el-form-item>
        <el-button size="mini" type="primary" class="seek el-button-primary" @click="search(searchData)">搜索</el-button>
        <el-button type="primary" size="mini" v-has="'customized:customTable:add'" class="add-staff btn-right el-button-primary" @click="addUnit">新建表格</el-button>
      </el-form>
    </div>
    <div class="tab-container">
      <div class="table-data" style="height: 80%">
        <el-table :data="tableData" stripe ref="tables" height="98%" style="width: 100%" v-loading="loading" :header-cell-style="{ background: '#f1f6ff' }" :cell-style="{ align: 'center' }" :tooltip-effect="'light'">
          ...
          <el-table-column align="center" prop="professionalTypeName" label="专业类别" min-width="120" show-overflow-tooltip v-if="headDate.orgType==5"></el-table-column>
          <el-table-column align="center" prop="tableName" label="表格名称" min-width="120"></el-table-column>
          <el-table-column align="center" prop="tableType" label="表格类型" min-width="120" v-if="headDate.orgType==5">
            <template slot-scope="scope">
              <span v-if="scope.row.tableType == 0">普通表格</span>
              <span v-if="scope.row.tableType == 1">质量评定表</span>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="sortval" label="排序值" min-width="60" show-overflow-tooltip></el-table-column>
          ...
          <el-table-column align="center" prop="alarmLevel" label="操作" min-width="220">
            <template slot-scope="scope">
              <!-- <el-button type="primary" plain size="mini" v-has="'system:form:query'"
              @click.native="examine(scope.row)">预览</el-button>
            <el-button size="mini" type="success" plain @click.native="edit(scope.row)" v-has="'system:form:edit'">编辑</el-button>
            <el-button type="danger" plain size="mini" v-has="'system:form:delete'"
              @click.native="remove(scope.row)">删除</el-button>
            <el-button type="primary" plain size="mini" v-has="'system:form:query'"
              @click.native="copyAdd(scope.row)">复制新增</el-button> -->
              <el-button type="primary" plain size="mini" v-has="'customized:customTable:query'" @click.native="examine(scope.row)">预览</el-button>
              <el-button plain type="success" size="mini" @click.native="edit(scope.row)" v-has="'customized:customTable:update'">编辑</el-button>
              <el-button plain type="danger" size="mini" v-has="'customized:customTable:delete'" @click.native="remove(scope.row)">删除</el-button>
              <el-button plain type="primary" size="mini" @click.native="copyAdd(scope.row)">复制新增</el-button>
            </template>
          </el-table-column>
          <div slot="empty" style="width: 100%">
            <img src="@/assets/wu.jpeg" style="width: 280px" />
          </div>
        </el-table>
      </div>
      ...
      <div style="display: none">
        <!-- style="display: none;" -->
        <div ref="printPagesd" id="printPagesd">
          <table style="border-collapse: collapse; overflow: auto; margin: 0 auto">
            <tr v-for="(item, idx) in tabulationArr" :key="idx" style="">
              <td style="box-sizing: content-box; border: none; margin: 0; padding: 0" @blur="tdBlur(item, id, $event.target.innerText, $event)" v-for="(i, id) in item.arr" :style="[i.style]" :key="id" :rowspan="i.row" :colspan="i.span">
                <div v-if="i.attributeTypes == 1 && i.checkout == false" :class="{ 'pre-wrap': i.name.indexOf('\n') != -1 }">
                  {{ i.name }}
                </div>
                <el-input v-if="i.attributeTypes == 1 && i.checkout == 1" disabled @input="tdNameChange(item, id, i.name)" @blur="tdNameBlur(item, id, i.name)" v-model="i.name" size="mini"></el-input>
    ...
    <!-- <el-dialog :title="addTitle" :visible.sync="show1" v-dialogDrag :close-on-click-modal="false"
      ...
      <div style="position: relative" class="dialog-form">
        <el-form :model="form" :rules="rules" ref="form" style="margin-top: 20px">
          <el-form-item label="工程大类" prop="fkEnginerringTypeId" :label-width="formLabelWidth">
            <el-select style="width: 100%" :disabled="showType == 4" v-model="form.fkEnginerringTypeId"
              @change="fkEnginerringChange" placeholder="请选择">
          ...
          <el-form-item label="专业类别" prop="fkProfessionalTypeId" :label-width="formLabelWidth">
            <el-select style="width: 100%" :disabled="showType == 4" v-model="form.fkProfessionalTypeId"
              placeholder="请选择">
              <el-option v-for="item in professionalType" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="表格类型" prop="tableType" :label-width="formLabelWidth">
            <el-select style="width: 100%" :disabled="showType == 4" v-model="form.tableType" placeholder="请选择">
          ...
          <el-form-item label="表格名称" prop="tableName" :label-width="formLabelWidth">
            <el-input v-model="form.tableName" autocomplete="off" :disabled="showType == 4"></el-input>
          </el-form-item>
          <el-form-item label="排序值" prop="sortval" :label-width="formLabelWidth">
            <el-input v-model="form.sortval" autocomplete="off" oninput="value=value.replace(/[^\d.]/g,'')"
              :disabled="showType == 4"></el-input>
          </el-form-item>
          <el-form-item label="备注" prop="remark" :label-width="formLabelWidth">
            <el-input v-model="form.remark" autocomplete="off" maxlength="100" :disabled="showType == 4"></el-input>
          </el-form-item>

          <el-form-item label="表格内容" prop="tableHtml" :label-width="formLabelWidth">
            <el-image v-if="srcList.length" style="width: 100px; height: 100px" :src="imageUrl"
              :preview-src-list="srcList"> </el-image>
            <div v-else style=" line-height: 100px;border: 1px solid #ccc;text-align: center;font-size:18px">未设计</div>
          </el-form-item>
        </el-form>
        <el-button size="mini" @click.native="design"
          style="position: absolute; left: 38px; bottom:38px;margin:0">设计表格</el-button>
    ...
    <el-dialog :title="addTitle" width="560px" :visible.sync="show1" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <div style="position: relative" class="dialog-form">
        <el-form :model="form" :rules="rules" ref="form">
          <el-form-item label="工程大类" prop="fkEnginerringTypeId" :label-width="formLabelWidth" v-if="headDate.orgType == 5">
            <el-select size="mini" style="width: 100%" disabled v-model="form.fkEnginerringTypeId" placeholder="请选择">
              <el-option v-for="item in projectType" :key="item.value" :label="item.label" :value="item.value"> </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="专业类别" prop="fkProfessionalTypeId" :label-width="formLabelWidth" v-if="headDate.orgType == 5">
            <el-select size="mini" style="width: 100%" :disabled="showType == 4" v-model="form.fkProfessionalTypeId" placeholder="请选择">
              <el-option v-for="item in itemType" :key="item.value" :label="item.label" :value="item.value"> </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="表格类型" prop="tableType" :label-width="formLabelWidth" v-if="headDate.orgType==5">
            <el-select size="mini" style="width: 100%" :disabled="showType == 4 || headDate.orgType != '5'" v-model="form.tableType" placeholder="请选择">
              <el-option label="普通表格" value="0"> </el-option>
              <el-option label="质量评定表" value="1"> </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="表格名称" prop="tableName" :label-width="formLabelWidth">
            <el-input size="mini" v-model="form.tableName" autocomplete="off" :disabled="showType == 4" maxlength="50"></el-input>
          </el-form-item>
          <el-form-item label="排序值" prop="sortval" :label-width="formLabelWidth">
            <el-input size="mini" v-model="form.sortval" autocomplete="off" type="number" @keydown.native="keyDown" oninput="if(value.length > 6) value=value.slice(0, 6)" :disabled="showType == 4"></el-input>
          </el-form-item>
          <el-form-item label="备注" prop="remark" :label-width="formLabelWidth">
            <el-input size="mini" v-model="form.remark" autocomplete="off" maxlength="100" :disabled="showType == 4"></el-input>
          </el-form-item>

          <el-form-item label="表格内容" prop="tableHtml" :label-width="formLabelWidth">
            <el-input size="mini" v-model="imageUrl" v-show="false" autocomplete="off"></el-input>
            <!-- <el-input v-model.trim="form.tableHtml" type="textarea" style="resize: none" :rows="5" disabled placeholder="请写入html表格代码" autocomplete="off"></el-input> -->
            <!-- <div ref="exportCanvas" id="exportCanvas"></div> -->
            <!-- <img :src="imageUrl" class="icon" crossOrigin="anonymous"></img> -->

            <el-image v-if="srcList.length" style="width: 100px; height: 100px" :src="imageUrl" :preview-src-list="srcList"> </el-image>
            <div v-else style="line-height: 100px; border: 1px solid #ccc; text-align: center; font-size: 18px">未设计</div>
          </el-form-item>
        </el-form>
        <el-button size="mini" @click.native="design" style="position: absolute; left: -7px; bottom: 38px; margin: 0">设计表格</el-button>
        <el-button size="mini" @click.native="preview" style="position: absolute; left: 5px; bottom: 0px">预览</el-button>
      </div>
      <div slot="footer" class="dialog-footer" v-show="showType != 4">
        <el-button size="mini" @click="show1 = false">取 消</el-button>
        <el-button size="mini" type="primary" class="el-button-primary" @click="submit">保 存</el-button>
      </div>
    </el-dialog>
    <el-dialog title="预览表格" :visible.sync="show2" :width="shortDmOrientation == 1 ? '880px' : '1300px'" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false">
      <div style="display: flex" class="dialog-form">
              ...
              <table style="border-collapse: collapse; margin-top: 2px; margin-left: 28px; background: #fff; color: #000" ref="printPage" id="printPage">
                <tr v-for="(item, idx) in tabulationArr" :key="idx" style="">
                  ...
                  <td style="border: none; margin: 0; padding: 0" @blur="tdBlur(item, id, $event.target.innerText, $event)" v-for="(i, id) in item.arr" :style="[i.style]" :key="id" :rowspan="i.row" :colspan="i.span">
                    ...
                    <!-- <el-select  clearable size="mini" v-model="i.name" disabled
                      placeholder="请选择">
        ...
        <div style="display: inline-block; margin-top: 20px; width: 150px">
          <el-button @click="print" type="primary" class="el-button-primary" size="mini">打印</el-button>
          <!-- 普通 -->
          ...
          <div style="margin-top: 8px">
            ...
            <!-- <el-button type="primary" class="el-button-primary" size="mini" style="margin:5px 0px 0px 0px">保存设置</el-button> -->
          </div>
        </div>
      </div>
    </el-dialog>
    <el-dialog title="边距设置" width="600px" :visible.sync="ranksEdit" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false">
      <div style="display: flex" class="dialog-form">
        <div style="margin: 100px 16px 0px 60px">左<el-input-number v-model="ranks.left" size="mini" :min="1" :max="200" label="描述文字"></el-input-number></div>
        <div>
          <div style="margin: 5px">上 <el-input-number v-model="ranks.top" size="mini" :min="1" :max="200" label="描述文字"></el-input-number></div>
          <div style="position: relative; width: 160px; height: 260px; border: 1px solid">
            <div class="left-xian" style="height: 100%; width: 1px; background: #000; position: absolute; left: 2px; top: 0" :style="'left:' + ranks.left + 'px'"></div>
            <div class="top-xian" style="width: 100%; height: 1px; background: #000; position: absolute; left: 0" :style="'top:' + ranks.top + 'px'"></div>
            <div class="right-xian" style="height: 100%; width: 1px; background: #000; position: absolute; top: 0" :style="'right:' + ranks.right + 'px'"></div>
            <div class="bottom-xian" style="width: 100%; height: 1px; background: #000; position: absolute; left: 0" :style="'bottom:' + ranks.bottom + 'px'"></div>
          </div>
          <div style="margin: 5px">下 <el-input-number v-model="ranks.bottom" size="mini" :min="0" :max="200" label="描述文字"></el-input-number></div>
        </div>
        <div style="margin: 5px">
          <div style="margin: 30px; margin-top: 100px">右 <el-input-number v-model="ranks.right" size="mini" :min="0" :max="200" label="描述文字"></el-input-number></div>
        </div>
      </div>
      <div slot="footer">
        <el-button size="mini" type="info" @click="ranksEdit = false">取消</el-button>
        <el-button type="primary" class="el-button-primary" @click="marginNumBer" size="mini">保 存</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    deviation(val) {
      const data1 = this.$refs.tableTd[this.ranksIndex * 30 - 5].offsetHeight;
      if (data1 > val) {
    ...
    searchSysAuditRole() {
      // 签名人：
      this.$api.customizedAuditRole({ workflowType: 9, fkOrgId: this.listQuery.fkOrgId }).then(res => {
        if (res.code == 200) {
    ...
    defer() {
      ...
      s = s + this.id;
      const i = this.tabulation[this.idx].arr[this.id];
      const wIndex = i.style.height.lastIndexOf("px");
    ...
    heightRightClick(idx, e, sta) {
      ...
      if (sta == 1) {
        ...
        const boxWidth = this.$refs.colWidthBox.getBoundingClientRect().width - letWidth;
      ...
      if (this.sta == 1) {
        ...
        this.tabulation[this.rightEditIdx].arr.forEach((item, e) => {
          if (this.rightEditIdx == 0) {
            ...
            // if (item.style.display == "none") {
            //   lineArr[e].style.display = "none";
            //   // this.tabulation[this.rightEditIdx-1].arr[e].
            //   let itemIdx = this.tabulation[this.tabulation[this.rightEditIdx-1].arr[e].mergeRow].arr[this.tabulation[this.rightEditIdx-1].arr[e].mergeSpan];
            //   if(itemIdx.row>1 && itemIdx.span>1){
            //     lineArr[e].span = itemIdx.span
            //   }
            // }
            //  console.log(this.tabulation[this.rightEditIdx-1].arr[e])
            // if( this.tabulation[this.rightEditIdx].arr[e])
            // if(this.tabulation[this.rightEditIdx-1].arr[e].style.display == "none"){
            // let itemIdx = this.tabulation[this.tabulation[this.rightEditIdx-1].arr[e].mergeRow].arr[this.tabulation[this.rightEditIdx-1].arr[e].mergeSpan];
            // console.log()
            // let rows
            // if(itemIdx.row>1 && rows != this.tabulation[this.rightEditIdx-1].arr[e].mergeSpan){
            //   itemIdx.row = itemIdx.row+1
            //   rows = e
            // }
            // if(this.tabulation[this.rightEditIdx-1].arr[e].row>1){
            //   this.tabulation[this.rightEditIdx-1].arr[e].row = this.tabulation[this.rightEditIdx-1].arr[e].row+1
            // }
            // if(itemIdx.row>1 && itemIdx.span>1 && rows != this.tabulation[this.rightEditIdx-1].arr[e].mergeSpan){
            //   lineArr[e].span = itemIdx.span
            // }
            // lineArr[e].style.display = "none";
            // }
        ...
        // this.tabulation[this.tabulation[this.rightEditIdx-1]].arr.forEach(item=>{
    ...
    highWide() {
      // this.$refs.colWidth.forEach((item, idx) => {
      //   console.log("===========================")
      //   console.log(item.style.width)
      //   console.log(this.tabulation[this.tabulation.length - 1].arr[idx].style.width)
      //   console.log("===========================")
      //   item.style.width = this.tabulation[this.tabulation.length - 1].arr[idx].style.width;
      // });
    ...
    widthClick(idx, event) {
      ...
      document.onmousemove = function (event) {
        ...
        // _this.tabulation.forEach(item => {
        //   if (item.arr[idx].style.display == "none") {
        //     let span = item.arr[idx].mergeSpan
        //     let wIndex = item.arr[span].style.width.lastIndexOf("px");
        //     let w = item.arr[span].style.width.substring(wIndex, item.arr[span].style.width);

        //     // item.arr[span].style.width = w + change + "px";
        //     console.log("----------------")
        //     console.log( w + change )
        //     console.log("----------------")
        //   }
        // });
        // for(let i = 0;i<_this.tabulation.length;i++){
        //   if (_this.tabulation[i].arr[idx].style.display == "none") {
        //     let span = _this.tabulation[i].arr[idx].mergeSpan
        //     let wIndex = _this.tabulation[i].arr[span].style.width.lastIndexOf("px");
        //     let w = _this.tabulation[i].arr[span].style.width.substring(wIndex, _this.tabulation[i].arr[span].style.width);
        //       if(_this.tabulation[i].arr[idx].mergeRow ==undefined || _this.tabulation[i].arr[idx].mergeRow ==i){
        //         _this.tabulation[i].arr[span].style.width =  Number(w) + change+"px"
        //       }
        //   }
        // }
        // _this.tabulation.forEach(item => {
    ...
    //   // html2canvas(this.$refs.tableDesign, {
    ...
    //   //   console.log("********************");
    //   //   // const link = document.createElement("exportCanvas");
      ...
      this.$api.searchTypeList().then(res => {
        if (res.code == 200) {
          ...
          res.data.forEach(item => {
            item.label = item.engTypeName;
            item.value = item.pkId;
            if (item.pkId == this.headDate.fkEnginerringTypeId) {
              data = item.typeSonList;
            }
          });
          // this.searchOptions[0].options = res.data;
          this.projectType = res.data;
    ...
    //         console.log(addForm.tableHtml);
    //         addForm.tableHtml = utils.zip(JSON.stringify(addForm.tableHtml));
    //         addForm.tableData = utils.zip(JSON.stringify(addForm.tableData));
    //         this.$api.addForm(addForm).then(res => {
    //           if (res.code == 200) {
    //             this.loading = false;
    //             this.show1 = false;
    //             this.pageForm();
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.showType == 1) {
            ...
            addForm.tableHtml = utils.zip(JSON.stringify(addForm.tableHtml));
            addForm.tableData = utils.zip(JSON.stringify(addForm.tableData));
            addForm.fkOrgId = this.headDate.pkId;
            if (this.headDate.orgType != 5) {
              addForm.fkProfessionalTypeId = "";
              addForm.fkEnginerringTypeId = "";
            }
            this.$api.addForm(addForm).then(res => {
              if (res.code == 200) {
                this.loading = false;
            ...
            if (this.headDate.orgType != 5) {
              data.fkProfessionalTypeId = 0;
              data.fkEnginerringTypeId = 0;
            }
            this.$api.modifyForm(data).then(res => {
              if (res.code == 200) {
                this.loading = false;
    ...
    // pageForm() {
    //   this.loading = true;
    //   this.$api.pageForm(this.listQuery).then(res => {
    //     if (res.code == 200) {
    ...
    pageForm() {
      ...
      console.log("========================");
      this.listQuery.fkOrgId = this.headDate.pkId;
      if (this.headDate.orgType == 5) {
        this.listQuery.fkEnginerringTypeId = this.headDate.fkEnginerringTypeId;
      }
      this.$api.pageForm(this.listQuery).then(res => {
        if (res.code == 200) {
          this.tableData = res.data.records;
    ...
    // 查看
    examine(row) {
      this.$api.queryForm({ pkId: row.pkId }).then(res => {
        if (res.code == 200) {
    ...
    //   this.form.fkEnginerringTypeId = rows.fkEnginerringTypeId;
    //   this.form.fkProfessionalTypeId = rows.fkProfessionalTypeId == 0 ? "" : rows.fkProfessionalTypeId;
    ...
    remove(row) {
      const _this = this;
      _this
        .$confirm("确定删除该表格信息？", "删除确认", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        })
        .then(() => {
          this.$api.deleteForm({ pkId: row.pkId }).then(res => {
            if (res.code == 200) {
              this.$message({
                type: "success",
                message: "删除成功！"
    ...
    addUnit() {
      ...
      // form.tableType
      if (this.headDate.orgType == 5) {
        this.form.fkEnginerringTypeId = this.headDate.fkEnginerringTypeId;
      } else {
        this.form.tableType = "0";
      }
      if (this.$refs.form != undefined) {
        this.$refs.form.resetFields();
      }
    ...
    //   console.log(rows.tableHtml);
    //   console.log("Xxxxxxxxxxxxx");
    //   this.form = {
    //     tableName: null, // 表格名称
    //     sortval: null, // 排序
    //     remark: null, // 备注
    //     tableHtml: utils.unzip(rows.tableHtml), // 表格内容
    //     fkEnginerringTypeId: null,
    //     fkProfessionalTypeId: "",
    //     tableType: null,
    //     tableHead:rows.tableHead,
    //     tableData: utils.unzip(rows.tableData)
    //   };
    //   this.shortDmOrientation = rows.tableHead;
    //   // return console.log(JSON.String(utils.unzip(row.tableHtml)))
    //   // this.form.tableHtml = JSON.String(utils.unzip(row.tableHtml));
    //   // this.form.tableData = JSON.String(utils.unzip(row.tableData));
    // },
    // copyAdd(row) {
    //   let rows = JSON.stringify(row);
    //   rows = JSON.parse(rows);
    ...
    copyAdd(row) {
      this.$api.queryForm({ pkId: row.pkId }).then(res => {
        if (res.code == 200) {
          ...
          console.log("Xxxxxxxxxxxxx");
          this.form = {
            tableName: row.tableName +"-复制", // 表格名称
            sortval: row.sortval, // 排序
            remark: row.remark, // 备注
            tableHtml: utils.unzip(rows.tableHtml), // 表格内容
            fkEnginerringTypeId: rows.fkEnginerringTypeId,
            fkProfessionalTypeId: rows.fkProfessionalTypeId,
    ...
    preview() {
      ...
      const tabulaArr = this.form.tableHtml;
      let x = 0;
      let y = 0;
      this.shortDmOrientation = this.form.tableHead;
      if (this.shortDmOrientation == 1) {
