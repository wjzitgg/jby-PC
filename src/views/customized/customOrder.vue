<template>
  <div class="app-container">
    <!-- 头部搜索 -->
    <custom style="height: 93%;" @choose="choose" v-show="!basicStatus" :types="'2'"></custom>
    ...
    <div class="search-bar-box" v-show="basicStatus">
      <el-form :model="searchData" class="form-data">
        ...
        <el-button type="primary" size="mini" v-has="'customized:customOrder:add'" class="add-staff btn-right el-button-primary" @click="addRecord">新增规则</el-button>
      </el-form>
    </div>
      ...
      <div class="table-data" style="height:81%">
        <el-table :data="tableData" stripe v-loading="loading" height="90%" :header-cell-style="{ background: '#f1f6ff' }">
          ...
          <el-table-column label="操作" align="center" width="400">
            <template slot-scope="{ row }">
              <el-button size="mini" plain type="primary" v-has="'customized:customOrder:query'" @click="examine(row.pkId)">查看</el-button>
              <el-button size="mini" plain type="success" v-has="'customized:customOrder:update'" @click="editOrder(row.pkId)">编辑</el-button>
              <el-button size="mini" plain type="warning" v-has="'customized:customOrder:configure'" @click="configuration(row)">配置单据</el-button>
              <el-button size="mini" plain type="danger" v-has="'customized:customOrder:delete'" @click="deleteOrder(row.pkId)">删除</el-button>
            </template>
          </el-table-column>
    ...
    <el-dialog title="配置单据" width="400px" :visible.sync="configurationShow" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <div class="dialog-form">
        <div class="rule">
          规则名称:<span class="weight-600">{{ configurationTitle }}</span>
        </div>
        <div class="ml-20">
          <el-checkbox-group v-model="checkList">
            <el-checkbox class="rule-check-item" v-for="(item, idx) in billsType" :key="idx" :disabled="item.disabled" :label="item.key" :value="item.key">{{ item.name }}</el-checkbox>
          </el-checkbox-group>
        </div>
      </div>
      <div slot="footer">
        <el-button size="mini" @click="configurationShow = false">取 消</el-button>
        <el-button size="mini" type="primary" class="el-button-primary" @click="ruleSubmit">保 存</el-button>
      </div>
    </el-dialog>
    <!-- 新增修改弹窗 -->
    <el-dialog :title="ruleTittle" width="600px" :visible.sync="ruleShow" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <el-form :model="ruleData" :rules="rules" ref="form" class="row-height dialog-form">
        <div>
          <el-row :span="24" class="el-row-data">
            <el-col :span="24">
              <el-form-item label="规则名称" prop="ruleName" label-width="110px">
                <el-input v-model="ruleData.ruleName" size="mini" maxlength="50"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="14">
          ...
          <div v-for="(item, idx) in ruleDetails" :key="idx" class="rule-numerical">
            ...
            <el-select v-model="item.ruleCode" size="mini" class="wh-150">
            ...
            <el-input v-if="item.ruleCode == 4 || item.ruleCode == 5 || item.ruleCode == 6 || item.ruleCode == 7" @blur="ruleHeadBlur(item)" @input="item.ruleHead = item.ruleHead.replace(/^(0+)|[^\d]+/g, '')" v-model="item.ruleHead" size="mini" class="wh-90"></el-input>
            <span v-if="item.ruleCode == 4 || item.ruleCode == 5 || item.ruleCode == 6 || item.ruleCode == 7">至</span>
            <el-input v-if="item.ruleCode == 4 || item.ruleCode == 5 || item.ruleCode == 6 || item.ruleCode == 7" @blur="ruleCodeBlur(item)" @input="item.ruleTail = item.ruleTail.replace(/^(0+)|[^\d]+/g, '')" v-model="item.ruleTail" size="mini" class="wh-90"></el-input>
      ...
      <div slot="footer">
        ...
        <el-button size="mini" type="primary" class="el-button-primary" @click="submitRule('form')">保 存</el-button>
      </div>
    </el-dialog>
    <!-- 查看弹窗 -->
    <el-dialog title="单号规则信息" width="600px" :visible.sync="examineShow" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <div class="dialog-form" style="padding-bottom: 20px;margin-top: 0;">
        ...
        <div v-show="tabPosition == 1" class="examineShow-box">
          ...
          <div v-for="(item, idx) in ruleDetails" :key="idx" class="rule-numerical">
            <span class="numerical-order">{{ idx + 1 }}</span>
            <el-select v-model="item.ruleCode" size="mini" class="wh-150" disabled>
              <el-option v-for="(item, idx) in ruleList" :key="idx" :label="item.keyVal" :value="item.keyName"></el-option>
            </el-select>
    ...
    <el-dialog title="选择企业" width="70%" :visible.sync="choiceShow" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <div style="height: 560px;">
        <custom @choose="choose" style="height: 85%;" :types="'2'" ref="custom" :status="choiceShow"></custom>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ruleHeadBlur(item) {
      if (Number(item.ruleTail) < Number(item.ruleHead)) {
        item.ruleTail = "";
      }
    },
    ruleCodeBlur(item) {
      if (Number(item.ruleTail) < Number(item.ruleHead)) {
        item.ruleTail = "";
        return this.$message.warning("结束位数不能小于开始位数");
      }
    },
    searchOrderTypeByOrgId() {
      this.$api.searchOrderTypeByOrgId({ orgId: this.headDate.pkId }).then(res => {
        if (res.code == 200) {
          /**
           * @return {*} 删除下标第一个数据测试  // this.billsType = res.data.shift();
           */
    ...
    // 配置单据
    configuration(row) {
      this.configurationTitle = row.ruleName;
      this.fkRuleId = row.pkId;
      this.billsType.forEach(item => {
        item.disabled = false;
      });
      this.$api.findByIdNoJurisdiction({ pkId: row.pkId }).then(res => {
        if (res.code == 200) {
          this.checkList = res.data.customOrderSets;
