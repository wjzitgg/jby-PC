      ...
      <template slot="left">
        <el-form ref="form" label-width="80px">
          ...
          <el-form-item label="标段项目" v-if="user.orgType != 5">
            ...
            <el-select v-model="inpData.fkProjectBidId" size="mini" class="wh-140" v-if="[6, 7].includes(user.orgType)">
              <el-option value="" label="全部"></el-option>
              <el-option v-for="item in options" :key="item.pkId" :value="item.pkId" :label="item.projectName"></el-option>
            </el-select>
            <proSelct @selectIds="selectIds" v-else></proSelct>
          </el-form-item>
          <!-- <el-form-item label="项目部" v-if="(user.orgType === 2 && id === 1) || (user.orgType === 4 && id === 1) || (user.orgType === 4 && id === 3) || (user.orgType === 6 && id === 3) || (user.orgType === 7 && id === 4) || (user.orgType === 4 && id === 4)">
            <el-select size="mini" v-model="inpData.fkOrgId">
              <el-option value="" label="全部"></el-option>
              <el-option v-for="item in options" :key="item.pkId" :value="item.pkId" :label="item.projectOrgName"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="监理公司" v-if="user.orgType === 2 && id == 2">
            <el-select size="mini" v-model="inpData.fkOrgId">
              <el-option value="" label="全部"></el-option>
              <el-option v-for="item in constructionUnits" :key="item.fkOrgId" :value="item.fkOrgId" :label="item.orgName"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="建设单位" v-if="user.orgType === 3 && id === 2">
            <el-select size="mini" v-model="inpData.fkCustomerId">
              <el-option value="" label="全部"></el-option>
              <el-option v-for="item in constructionUnits" :key="item.pkId" :value="item.pkId" :label="item.customName"></el-option>
            </el-select>
          </el-form-item> -->
          <el-form-item label="合同状态" v-if="typeShow">
          ...
          <el-form-item label="合同名称" v-if="typeShow">
            <el-input size="mini" v-model="inpData.contractName" maxlength="50"></el-input>
          </el-form-item>
          <el-button size="mini" v-if="typeShow" type="primary" icon="el-icon-search" class="btn-serach el-button-primary" @click="search">查询</el-button>
        </el-form>
      </template>
      <div slot="right" v-show="user.orgType === 5 || user.orgType === 3" v-if="typeShow">
        <el-button type="primary" class="el-button-primary" size="mini" v-has="jurisdiction.add" @click="handleOpen(2)">新增合同</el-button>
      </div>
    </SearchBar>
    <div class="table-data" style="height: 87%; margin-top: 10px" v-if="typeShow">
      <!-- 表格 -->
      <el-table :data="tableData" v-loading="loading" ref="tables" stripe height="90%" :header-cell-style="{ background: '#f1f6ff' }" :key="randomKey" :tooltip-effect="'light'" :summary-method="getSummaries" show-summary>
        ...
        <el-table-column align="center" prop="customerOrgName" label="建设单位" v-if="id === 1 || (id === 2 && user.orgType === 3)" show-overflow-tooltip> </el-table-column>
        ...
        <el-table-column align="center" label="操作" width="400">
          <template slot-scope="{ row }">
            <el-button plain size="mini" @click="handleOpen(1, row)" type="primary" v-has="jurisdiction.query">查看</el-button>
            <el-button plain size="mini" @click="handleOpen(3, row)" v-show="row.editStatus == '1'" v-if="!row.enableStatus" v-has="jurisdiction.edit" type="success">编辑</el-button>
            ...
            <el-button plain size="mini" v-if="row.attachments && row.attachments.length && !row.enableStatus" v-has="jurisdiction.download" @click="handleDownload(row.attachments, row.contractName)" type="success">下载附件</el-button>
            <el-button plain size="mini" @click="handleDel(row)" v-show="row.deleteStatus == '1'" v-if="!row.enableStatus" v-has="jurisdiction.delete" type="danger">删除</el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  // },
  mounted() {
    // if ((this.user.orgType === 2 && this.id === 1) || (this.user.orgType === 4 && this.id === 1) || (this.user.orgType === 4 && this.id === 3) || (this.user.orgType === 6 && this.id === 3) || (this.user.orgType === 7 && this.id === 4) || (this.user.orgType === 4 && this.id === 4)) {
    //   this.getItemListByCustom();
    // } else if ((this.user.orgType === 2 && this.id == 2) || (this.user.orgType === 3 && this.id === 2)) {
  ...
  methods: {
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (column.label == "合同金额(元)") {
          // this.amount
          sums[index] = !this.pricePermission ? "***" : this.amount;
        }
      });
    ...
    handleDownload(row, name) {
      ...
      if (row.length === 1) {
        console.log(1);
        const exit = row[0].attachUrl.substring(row[0].attachUrl.lastIndexOf(".") + 1);
        this.fileLinkToStreamDownload(row[0].attachUrl, row[0].attachName, exit);
        this.loading = false;
      } else if (row.length > 1) {
        // const fileUrls = row.map(item => item.attachUrl).join(",");
        let fileList = row.map(item => ({ fileUrl: item.attachUrl, fileName: item.attachName }));
        this.$api.compressFile({ fileName: "附件" + Date.now(), fileList }).then(res => {
          if (res.code === 200) {
            this.fileLinkToStreamDownload(res.data, name + "-附件", "zip");
          } else {
            this.$message.warning(res.msg);
          }
          this.loading = false;
    ...
    handleDel(row) {
      this.$confirm("确定删除该合同信息?", "删除确认", {
        ...
        this.$api.delContractById({ pkId: row.pkId }).then(res => {
          if (res.code == 200) {
            this.$message({
              type: "success",
              message: "删除成功!"
    ...
    termination(row) {
      this.$confirm("确定终止该合同？", "终止合同确认", {
        ...
        this.$api.terminationConstant({ pkId: row.pkId }).then(res => {
          if (res.code == 200) {
            this.getContractPageListByCondition();
            this.$message({
              type: "success",
    ...
    getContractPageListByCondition() {
      ...
      this.$api.getContractPageListByCondition(data).then(res => {
        if (res.code == 200) {
          this.loading = false;
  ...
  computed: {
    ...
    jurisdiction() {
      if (this.id === 1) {
        ...
        return {
          query: "contract:supervisor:query",
          add: "contract:supervisor:add",
          edit: "contract:supervisor:edit",
          delete: "contract:supervisor:delete",
          download: "contract:supervisor:download",
          termination: "contract:supervisor:termination"
        };
      } else if (this.id === 3) {
        return {
