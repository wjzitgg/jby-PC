<template>
  <div class="app-container">
    ...
    <!-- 表ge -->
    <div class="table-data" style="height:75%" v-if="typeShow">
      ...
      <div class="acceptDia">
        ...
        <div class="dialog-head" :key="dialogVisible">
          <el-row :gutter="24" style="width:100%">
            ...
            <el-col :span="4" v-if="bizType==1" :class="{'mt-10':bizType==1&&form.enableStatus !='待审批'}">
              <span>分项工程：</span>
              <div class="content" :title="form.fkItemName">{{ form.fkItemName }}</div>
            </el-col>
            <el-col :span="4" :class="{'mt-10':bizType==1&&form.enableStatus !='待审批'}">
              <span>申请人：</span>
            ...
            <template v-if="form.enableStatus !='待审批'">
              ...
              <el-col :span="4" class="mt-10">
                <span>审批状态：</span>
                <div class="content" :title="form.enableStatusName">{{ form.enableStatusName }}</div>
              </el-col>
        ...
        <examine-bpmn :fkProfessionalTypeId="form.fkProfessionalTypeId" v-if="bizType!=1"
          style="width: 100%; height: 500px" :workflowType="bizType" :fkOrgId="user.fkOrgId" ref="examineBpmn"
          :XmlStr="XmlStr" :tableSource="'1'" :dataList="form.workflowTemplateVo" :fkWorkflowId="form.fkWorkflowId"></examine-bpmn>
        </div>
...
export default {
  ...
  methods: {
    ...
    searchItemByTemplate(){
      ...
      this.$api.searchItemByTemplate({pkIds}).then(res=>{
        if(res.code===200){
    ...
    findProdItemApprovalById(pkId){
      this.$api.findProdItemApprovalById({pkId}).then(res=>{
        this.loading = false;
        if(res.code==200){
          this.form = res.data;
    ...
    searchProdItemApprovalPage() {
      ...
      this.$api.searchProdItemApprovalPage(data).then(res => {
        if (res.code === 200) {
          if (res.data) {
    ...
    openDialog(row, type) {
      ...
      this.loading = true;
      this.$api
        .findProdItemApprovalById({ pkId: row.pkId })
        .then(res => {
          if (res.code == 200) {
            if (res.data) {
              this.form = res.data;
    ...
    withdrawActivity() {
      ...
      // }).then(({ value }) => {
        ...
        this.loading = true;
        this.$api
          .approveProdItemApprovalExamine(arr)
          .then(res => {
            this.loading = false;
            if (res.code == 200) {
