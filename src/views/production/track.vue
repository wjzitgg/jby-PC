<template>
  <div class="app-container">
    ...
    <div slot="header" class="clearfix card-header">
      <div class="head">
        <el-form ref="form" label-width="80px">
          ...
          <el-form-item :label="labelName" v-if="[5, 7].includes(user.orgType)">
            <el-select v-model="fkProjectId" size="mini" @change="proChange" v-if="topRadio > 2 && [2, 3, 9].includes(user.orgType)">
          ...
          <el-form-item label="检查状态" v-if="typeShow">
            <el-select v-model="searchData.finishStatus" size="mini">
          ...
          <el-form-item label="检查结果" v-if="typeShow">
            <el-select v-model="searchData.checkResult" size="mini">
              <el-option value="" label="全部"></el-option>
          ...
          <el-form-item label="项目名称" v-if="typeShow">
            <el-input size="mini" v-model.trim="searchData.itemName" maxlength="100"></el-input>
          </el-form-item>
    ...
    <!-- 表单 -->
    <div class="table-data" style="height: 78%" v-if="typeShow">
      <el-table :tooltip-effect="'light'" :data="tableData" ref="tables" height="100%" @expand-change="expandChange" :tree-props="{ children: 'children' }" row-key="pkId" :row-class-name="tableRowClassName" stripe :header-cell-style="{ background: '#f1f6ff' }" v-loading="loading2">
        ...
        <el-table-column align="center" label="分包单位" prop="customerName" v-if="topRadio != 2" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" label="工区" prop="areaName" v-if="topRadio != 1" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" label="生产验收流程" prop="workflowName" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column align="center" label="检查结果" show-overflow-tooltip>
          <template slot-scope="{ row }">
            <span v-if="(row.grade == 3 && row.isOriginal == 1 && row.itemType != 0) || (row.grade == 3 && row.itemType == 0)">{{ row.checkResult === 0 ? "不合格" : row.checkResult === 1 ? "合格" : row.checkResult === 2 ? "良好" : "" }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="操作">
          <template slot-scope="{ row }">
            <el-button size="mini" type="primary" plain @click="openDialog(row.pkId)" v-if="!!row.detailBtnStatus">检查详情</el-button>
          </template>
        </el-table-column>
    ...
    <proCheckDialog :data="disposeData" :proChangeDialog.sync="proChangeDialog" :appDiaType="1" />
    <!-- 节点信息模态框 -->
    <!-- <NodeDialog :nodeDialog.sync="nodeDialog" :data="disposeData" :roleDtoName="''" :disabled="true" :comment="comment" @cleanData="cleanData" /> -->
    <!-- 预览pdf弹框 -->
...
export default {
  components: { Pagination, NodeDialog, pdfPiev, proCheckDialog, treeSelect },
  data() {
    return {
      // 主表格数据
      tableData: [],
  ...
  methods: {
        ...
        .then(res => {
          if (res.code === 200) {
            if (res.data) {
              ...
              this.disposeData.workflowTableList.forEach(item => {
    ...
    // 获取项目部
    getItemListByCustom() {
      const api = [2, 3, 9].includes(this.user.orgType) ? this.$api.getAllProjectInfo : this.$api.getItemListByCustom;
      api().then(res => {
        if (res.code === 200) {
    ...
    search() {
      ...
      // this.page.pageNum = 1;
      this.getQualityAndSecurity();
      // this.searchQualityAndSecurity(this.searchData)
    },
    // 打开模态框
    openDialog(fkItemId) {
      this.bpmnImg = "";
    ...
    // 打开节点信息模态框
    openNodeDialog(nodeId, comment) {
      this.lookOver(nodeId);
    ...
    lookOver(nodeId) {
      ...
      this.$api
        .getDoneTaskDetail({
          flowCaseId: this.itemData.fkFlowCaseId,
          nodeId
        })
        .then(res => {
          if (res.code === 200) {
  ...
  computed: {
    ...
    showList() {
      const { itemName, checkResult, finishStatus } = this.inpData;
      if (!itemName && checkResult === "" && !finishStatus) {
        return this.tableData;
      } else {
        if (checkResult === 0) {
          return this.tableData.filter(item => item.itemName.indexOf(itemName) !== -1 && String(item.completionStatus).indexOf(finishStatus) !== -1 && item.checkResult === checkResult);
        } else {
          const arr = this.tableData.filter(item => item.itemName.indexOf(itemName) !== -1 && String(item.checkResult).indexOf(checkResult) !== -1 && String(item.completionStatus).indexOf(finishStatus) !== -1);
          return arr;
        }
      }
    }
