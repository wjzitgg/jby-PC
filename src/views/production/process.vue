<template>
  <div class="app-container" v-loading="loading2">
    ...
    <div class="clearfix card-header search-mr">
      <div class="head">
        <el-form ref="form" label-width="80px">
          ...
          <el-form-item label="发起人" v-if="typeShow">
          ...
          <el-button size="mini" v-if="typeShow" class="btn-serach el-button-primary" type="primary" icon="el-icon-search" @click="search">查询</el-button>
        </el-form>
        <el-button type="primary" v-if="typeShow" size="mini" class="launch el-button-primary" @click="faqiLc" v-has="'production:process:add'">发起流程</el-button>
      </div>
    </div>
    <!-- 表ge -->
        ...
        <el-table-column label="操作" align="center" width="320">
          <template slot-scope="{ row }">
            ...
            <div class="chakan">
              ...
              <el-button plain type="primary" size="mini" @click="lookOver(row, 1, 2)" v-has="'production:process:approveQuery'">查看</el-button>
              <el-button plain type="danger" size="mini" @click="clearLog(row.pkId)" v-if="row.clearBtnStatus == 1">清除记录</el-button>

              <el-button plain size="mini" type="success" v-if="row.handleStatus == 1" @click="lookOver(row, 2, 3)">审批</el-button>
              ...
              <el-button plain size="mini" v-show="row.editStatus == '1'" v-if="row.approveStatus == 0 || row.approveStatus == 3 || row.approveStatus == 7" @click="lookOver(row, 1, 1)" v-has="'production:process:edit'" type="success">{{ row.approveStatus == 3 || row.approveStatus == 7 ? "重新发起" : "编辑" }}</el-button>
    ...
    <!-- 分页组件 -->
    <Pagination v-if="typeShow" class="pagination" :currentPage="pageNum" :pageSize="pageSize" :total="total" @handleSizeChange="handleSizeChange" @handleCurrentChange="handleCurrentChange" />
    <!-- 发起流程弹窗 编辑-->
    <el-dialog :close-on-press-escape="false" :title="titles" width="70%" class="tree-dialog" top="5vh" :visible.sync="popup" v-dialogDrag :close-on-click-modal="false" v-loading="loading">
      ...
      <div v-show="step == 1" class="box">
        ...
        <el-tabs v-model="activeName4" type="card" v-else>
          <el-tab-pane label="审批人" name="approver">
            <div class="approval-dialog" :style="{ overflow: approvalData.launchType == 2 && !approvalData.workflowNodeDTOS[1].baseSubWorkflow.workflowNodeDTOS[1].prodSysRoleVo.selectedUserId ? 'hidden' : 'auto' }">
              <div class="noAppMask" v-if="approvalData.launchType == 2 && !approvalData.workflowNodeDTOS[1].baseSubWorkflow.workflowNodeDTOS[1].prodSysRoleVo.selectedUserId">此流程节点只能由【{{ approvalData.workflowNodeDTOS[1].baseSubWorkflow.workflowNodeDTOS[1].prodSysRoleVo.roleName }}】发起</div>
              <div class="begin">开始</div>
              <div class="begin-icon">
                <i class="el-icon-bottom"></i>
              </div>
              <div class="approver-workflowName">{{ approvalData.workflowName }}</div>
              <div v-for="(item, index) in approvalData.workflowNodeDTOS" :key="index" style="position: relative;">
                <i class="el-icon-bottom arrows-bottom" v-if="item.nodeType == 3" style=""></i>
                <div v-if="item.nodeType == 3" class="acceptance-node" :class="{ 'jun-center': item.baseSubWorkflow.workflowNodeDTOS.length < 8 }">
                  ...
                  <div style="display: inline-block;" v-for="(items, indx) in item.baseSubWorkflow.workflowNodeDTOS" :key="indx">
                    <div class="child-node" v-if="items.nodeType == 2">
                      ...
                      <div class="approval-select">
                        <el-select size="mini" :disabled="items.prodSysRoleVo.sysUserList.length == 0 || (launchType == 2 && index == 1 && indx == 1)" clearable :class="{ 'background-red': items.prodSysRoleVo.sysUserList.length == 0 || (launchType == 2 && index == 1 && indx == 1 && !items.prodSysRoleVo.selectedUserId) }" v-model="items.prodSysRoleVo.selectedUserId" @change="selectedUserIdChange">
                          <el-option v-for="(e, idx) in items.prodSysRoleVo.sysUserList" :key="idx" :label="e.userName" :value="e.pkId"> </el-option>
                          <el-option v-if="items.prodSysRoleVo.sysUserList.length == 0 || (launchType == 2 && index == 1 && indx == 1 && items.prodSysRoleVo.selectedUserId)" label="未找到匹配人员" value=""></el-option>
                        </el-select>
                      </div>
                    </div>
                  </div>
        ...
        <div>
          ...
          <div class="project-list" v-if="itemType > 0">
            ...
            <div class="list-item" v-if="itemType === 2">
              ...
              <div class="item-detels">
                <el-input :value="((footage - footage2).toFixed(2))-0" size="mini" type="number" @input="feedin" class="wh-140" disabled>
                  <template slot="append">米</template>
                </el-input>
              </div>
            </div>
            <div class="list-item" style="flex:1.55;">
              ...
              <el-tooltip effect="dark" :content="finishPec" placement="bottom-start" :disabled="isShowTooltip" class="item-detels2">
                <div @mouseenter="onMouseEnter(`finishPec`)">
                  <span ref="finishPec">{{ itemType !== 2 || (!!footage2 && itemType === 2) ? finishPec : "" }}</span>
                </div>
              </el-tooltip>
            </div>
          </div>
                  ...
                  <tr v-for="(item, idx) in item.tableHtml" :key="idx" v-show="item.status">
                    <td style="box-sizing: content-box;border:none;margin:0;padding:0" :contentEditable="i.attributeTypes == 1 && i.disabled == true && i.checkout == false && workflowTableList[index].editFlag == 1" @blur="tdBlur(item, id, $event.target.innerText, $event, i)" v-for="(i, id) in item.arr" :style="[i.style]" :key="id" :rowspan="i.row" :colspan="i.span">
                      ...
                      <span v-if="i.attributeTypes == 3"
                        ...
                        <img width="100px" crossorigin="anonymous" :src="i.signatureDate.img" alt="" />
      ...
      <span slot="footer" class="dialog-footer" v-show="step == 2">
        ...
        <el-button type="primary" class="el-button-primary" @click="checkCode(0)" v-if="!siStatus">发起流程</el-button>
    ...
    <approveCodeDialog ref="approveCodeDialog" :siStatus="siStatus" @signature="signature" :signed="signed" :approveCodeDialog.sync="approveCodeDialog" :sign-date="signDate" @closeLoading="closeLoading" @approveOk="approveOk" :noApprove="codeType == 0 ? false : true" :biz-type="0" :appCodeType="appCodeType" @getNew="getNew" />
  </div>
</template>
...
export default {
  components: { Pagination, NodeDialog, basicViwr, pdfPiev, chapterProcess, chapterProcessApproval, proCheckDialog, approveCodeDialog },
  data() {
    return {
      // 树形数据
      dataList: [],
  ...
  methods: {
    ...
    approveOk(unique) {
      if (this.codeType == 0) {
        this.loading = false
        this.launchFlow(1, unique);
      } else if (this.codeType == 1) {
        this.loading2 = true;
        this.$api
          .deleteProduction2({ pkId: this.clearId, unique })
          .then(res => {
            if (res.code === 200) {
    ...
    nextStep() {
      ...
      if (this.launchType == 2) {
        // 判断第一个节点是否没审批人
        if (!this.approvalData.workflowNodeDTOS[1].baseSubWorkflow.workflowNodeDTOS[1].prodSysRoleVo.selectedUserId) {
          return this.$message.warning("该流程首个发起节点必须有人");
        }
      }
      // 获取工序ID
    ...
    getWorkflowTables() {
      ...
      const data = {
        itemIdList: this.haschecked.join(",")
      };
      this.$api.getWorkflowTables(data).then(res => {
        if (res.code === 200) {
          // 处理表格
    ...
    revocation(row) {
      this.loading2 = true;
      this.$api
        .queryProduction({ approveId: row.pkId, type: 2 })
        .then(res => {
          this.loading2 = false;
          if (res.code == 200) {
    ...
    approverSetting() {
      ...
      this.$api.getWorkflowDetail(data).then(res => {
        if (res.code == 200) {
          this.launchType = res.data.launchType;
          res.data.workflowNodeDTOS.forEach((item, index) => {
            if (item.nodeType == 3) {
              item.baseSubWorkflow.workflowNodeDTOS.forEach((items, index2) => {
                if (items.nodeType == 2) {
                  if (this.launchType == 2 && index == 1 && index2 == 1) {
                    let arr = items.prodSysRoleVo.sysUserList.filter(item2 => item2.pkId == this.user.pkId);
                    items.prodSysRoleVo.selectedUserId = arr.length ? this.user.pkId : "";
                    this.signDate = {
                      fkNodeId: items.pkId,
                      fkTemplateId: items.fkWorkflowTemplateId
                    };
                  } else {
    ...
    getData() {
      ...
      this.loading = true;
      this.$api
        .queryApprovalFlow({ ...params, ...this.searchData })
        .then(res => {
          if (res.code === 200) {
    ...
    checked(data, node) {
      ...
      // 变量selectedId 没有选择
      if (this.selectedId == "") {
        this.selectedId = data.fkParentId;
        this.specialType = data.itemType;
      } else {
        let arr = this.$refs.treeForm.getCheckedNodes(true).filter(item => item.pkId != data.pkId);
        if (arr.length) {
          if (arr[0].itemType !== 0) {
        ...
        if (data.itemType !== 0) {
          ...
          // 普通类型
          // if (data.fkParentId != this.selectedId) {
          //   this.$refs.treeForm.setChecked(data, false);
          //   this.$message.warning("不同工序流程类型不能合并审批");
          // }
        }
    ...
    treeConfirm() {
      ...
      if (this.treeNode.length == 0) {
        console.log("选择为空了");
        this.treeId = ""; // 清空已勾选的分项
        this.nodeDetels = {}; // 清空
        this.approvalData = []; // 清空审批人
        return true;
      }
      // 确定再进来回显问题
      const arr = [];
      ...
      const fkSysItemId = this.treeNode.map(item => item.pkId).join(",");
      console.log(fkSysItemId);
            ...
            if (this.editNum == 1) {
              if (this.itemType == 2 && this.footage2 !== "") {
                const params = {
                  ...
                  itemName: this.nodeDetels.surplusItemName
                };
                this.$api
    ...
    launchFlow(num, unique) {
      ...
      // 路基
      if (this.itemType == 1) {
        if (this.footage == '' || this.footage == 0) {
          return this.$message.warning("剩余最大填高不能小于0米");
        }
        if (this.footage3 == '' || this.footage3 == 0) {
      ...
      if (this.itemType == 2) {
        ...
        if (this.footage2 == '' || this.footage2 == 0) {
    ...
    lookOver(row, types, num) {
      ...
      // 查询流程信息
      let type = types;
      if (num == 1 && (row.approveStatus == 3 || row.approveStatus == 7)) {
        type = 2;
      }
      this.loading2 = true;
      this.$api
        .queryProduction({ approveId: row.pkId, type })
        .then(res => {
          if (res.code === 200) {
            ...
            if (num == 1) {
              ...
              this.haschecked = [];
              this.haschecked = node_id.split(",");
              console.log("编辑的时候勾选ID", this.haschecked);
    ...
    withdraw(id) {
      this.loading2 = true;
      this.$api
        .queryProduction({ approveId: id, type: 2 })
        .then(res => {
          this.loading2 = false;
          if (res.code == 200) {
            this.disposeData = res.data;
...
.approval-dialog {
  ...
  .acceptance-node {
    ...
    scrollbar-color: #aaa #f5f5f5; /* 滚动条颜色 */
  }

  /* 定义滚动条样式，这里使用较细的滚动条 */
  .acceptance-node::-webkit-scrollbar {
    width: 8px;
    height: 8px;
  }

  .acceptance-node::-webkit-scrollbar-thumb {
    background-color: #aaa;
  }

  .acceptance-node::-webkit-scrollbar-thumb:hover {
