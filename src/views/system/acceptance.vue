<template>
  <div class="app-container">
    ...
    <el-button type="primary" size="mini" class="search-mr el-button-primary" @click="addUnit"
      v-has="'system:acceptance:add'">上传验收标准</el-button>
    <div class="tab-container">
      <div class="table-data" style="height: 82%;margin-top: 10px;">
        <el-table :data="tableData" height="95%" ref="tables" stripe style="width: 100%" v-loading="loading"
          :header-cell-style="{ background: '#f1f6ff' }" :cell-style="{ align: 'center' }" :tooltip-effect="'light'">
          ...
          <el-table-column align="center" prop="bookName" label="验收标准" min-width="180"></el-table-column>
          ...
          <el-table-column align="center" prop="alarmLevel" label="操作" fixed="right" min-width="320">
            <template slot-scope="scope">
              <el-button size="mini" plain type="primary" v-has="'system:acceptance:query'"
                @click.native="preview(scope.row)">预览</el-button>
              <el-button size="mini" type="success" plain v-has="'system:acceptance:edit'" @click.native="edit(scope.row)">编辑</el-button>
              <el-button type="success" plain size="mini" v-has="'system:acceptance:download'"
                @click.native="download(scope.row)">下载</el-button>
              <el-button type="danger" plain size="mini" v-has="'system:acceptance:delete'"
    ...
    <el-dialog :title="addTitle" width="500px" :visible.sync="show1" v-dialogDrag :close-on-click-modal="false" @close="close1"
      ...
      <el-form :model="form" ref="form" :rules="rules" class="dialog-form">
        <el-form-item label="工程大类" prop="fkEnginerringTypeId" :label-width="formLabelWidth">
          <el-select size="mini" style="width: 100%" placeholder="请选择" v-model="form.fkEnginerringTypeId"
        ...
        <el-form-item label="验收标准" prop="bookName" :label-width="formLabelWidth">
        ...
        <el-form-item label="pdf文件" prop="bookUrl" :label-width="formLabelWidth">
          <div>
            <!-- <el-upload
              class="upload-demo"
              ref="upload"
              action="https://epms-1307524156.cos.ap-guangzhou.myqcloud.com"
            ...
            >
              <el-button slot="trigger" size="small" type="primary"
                >选取文件</el-button
              >
              <span v-if="fileList.length == 0">未选择任何文件</span>
            </el-upload> -->
            <p class="fileT" @click="empty" v-show="form.bookUrl == null || form.bookUrl == ''">
              <input accept=".pdf" type="file" @change="upLoad" ref="file" id="upload" />
            </p>
...
export default {
  ...
  updated() {
    ...
    // let searchBarHeight = this.$refs.searchBar.$el.clientHeight
    // this.maxHeight = mainHeight - searchBarHeight - 86;
  },
  mounted() {
    this.searchTypeList();
    this.conditionPdf();
  ...
  methods: {
    ...
    // 获取PDF总页数
    getNumPages() {
      const loadingTask = pdf.createLoadingTask(this.form.bookUrl);
      loadingTask.promise
        .then(pdf => {
    ...
    edit(row) {
      ...
      const rowDate = JSON.stringify(row);
      this.show1 = true;
      this.showType = 2;
      this.addTitle = "编辑验收标准";
    ...
    download(row) {
      ...
      const url = row.bookUrl;
      const name = row.bookName;
