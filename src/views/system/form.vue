<template>
  <div class="app-container">
    <search-bar @search="search" :options="searchOptions" :total="totalCount" labelWidth="8em" ref="searchBar" :showHandler="false" @change="change"></search-bar>
    <el-button type="primary" size="mini" class="search-mr el-button-primary" @click="addUnit" v-has="'system:form:add'">新建表格</el-button>
    <div class="tab-container">
      <div class="table-data" style="height: 82%; margin-top: 10px">
        <el-table :data="tableData" stripe ref="tables" height="98%" style="width: 100%" v-loading="loading" :tooltip-effect="'light'" :header-cell-style="{ background: '#f1f6ff' }" :cell-style="{ align: 'center' }">
          ...
          <el-table-column align="center" prop="alarmLevel" label="操作" min-width="220">
            <template slot-scope="scope">
              <el-button type="primary" plain size="mini" v-has="'system:form:query'" @click.native="examine(scope.row)">预览</el-button>
              <el-button size="mini" type="success" plain @click.native="edit(scope.row)" v-has="'system:form:edit'">编辑</el-button>
              <el-button type="danger" plain size="mini" v-has="'system:form:delete'" @click.native="remove(scope.row)">删除</el-button>
              <el-button type="primary" plain size="mini" v-has="'system:form:query'" @click.native="copyAdd(scope.row)">复制新增</el-button>
            </template>
          </el-table-column>
    ...
    <el-dialog :title="addTitle" width="560px" :visible.sync="show1" :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <div style="position: relative" class="dialog-form">
        <el-form :model="form" :rules="rules" ref="form" style="margin-top: 20px">
          ...
          <el-form-item label="工程大类" prop="fkEnginerringTypeId" :label-width="formLabelWidth">
            <el-select size="mini" style="width: 100%" :disabled="showType == 4" v-model="form.fkEnginerringTypeId" @change="fkEnginerringChange" placeholder="请选择">
          ...
          <el-form-item label="专业类别" prop="fkProfessionalTypeId" :label-width="formLabelWidth">
            <el-select size="mini" style="width: 100%" :disabled="showType == 4" v-model="form.fkProfessionalTypeId" placeholder="请选择">
              <el-option v-for="item in professionalType" :key="item.value" :label="item.label" :value="item.value"> </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="表格类型" prop="tableType" :label-width="formLabelWidth">
            <el-select size="mini" style="width: 100%" :disabled="showType == 4" v-model="form.tableType" placeholder="请选择">
          ...
          <el-form-item label="表格名称" prop="tableName" :label-width="formLabelWidth">
            <el-input size="mini" v-model="form.tableName" maxlength="50" autocomplete="off" :disabled="showType == 4"></el-input>
          </el-form-item>
          <el-form-item label="排序值" prop="sortval" :label-width="formLabelWidth">
            <el-input size="mini" v-model="form.sortval" autocomplete="off" @keydown.native="keyDown" type="number" oninput="if(value.length > 6) value=value.slice(0, 6)" :disabled="showType == 4"></el-input>
          </el-form-item>
          ...
          <el-form-item label="表格内容" prop="tableHtml" :label-width="formLabelWidth">
            ...
            <div v-else style="line-height: 100px; border: 1px solid #ccc; text-align: center; font-size: 18px; border-radius: 5px">未设计</div>
          </el-form-item>
        </el-form>
        <el-button size="mini" @click.native="design" style="position: absolute; left: 5px; bottom: 38px; margin: 0">设计表格</el-button>
        <el-button size="mini" @click.native="preview" style="position: absolute; left: 18px; bottom: 0px">预览</el-button>
      </div>
      <div slot="footer" class="dialog-footer" v-show="showType != 4">
        <el-button size="mini" @click="show1 = false">取 消</el-button>
        <el-button size="mini" class="el-button-primary" type="primary" @click="submit">保 存</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog title="边距设置" width="600px" :visible.sync="ranksEdit" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false">
      <div style="display: flex" class="dialog-form">
        ...
        <div>
          <div style="margin: 5px">上 <el-input-number v-model="ranks.top" size="mini" :min="1" :max="30" label="描述文字"></el-input-number></div>
    ...
    <div ref="previewTableImg" style="padding-top: 20px; position: fixed; left: -99999px" :key="ranKey">
...
export default {
  ...
  methods: {
    ...
    searchSysAuditRole() {
      this.$api.searchSysAuditRole({ workflowType: 9 }).then(res => {
        if (res.code == 200) {
      ...
      this.$api.queryForm({ pkId: row.pkId }).then(res => {
        if (res.code == 200) {
          ...
          this.form = {
            ...
            tableHtml: utils.unzip(rows.tableHtml), // 表格内容
            fkEnginerringTypeId: row.fkEnginerringTypeId,
