<template>
  <div class="app-container">
    ...
    <div slot="header" class="card-header">
      <div class="head" v-if="user.orgType == 5">
      ...
      <div class="head head2" v-else>
        <div style="margin-right:6px">选择标段</div>
        <treeSelect @selectIds="selectIds"></treeSelect>
      </div>
      <!-- 新增按钮 -->
      <el-button type="primary" class="addBtn el-button-primary" @click="handleOpen(1)" size="mini" v-has="'project:item:add'">新增</el-button>
    </div>
    <!-- 表格 -->
      ...
      <el-table ref="tables" :data="tableData" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }" :tooltip-effect="'light'">
        ...
        <el-table-column align="center" label="操作" width="250">
          <template slot-scope="{ row }">
            <el-button size="mini" plain type="primary" @click="handleOpen(2, row.pkId)" v-has="'project:item:query'">查看</el-button>
            <el-button size="mini" plain type="success" @click="handleOpen(1, row.pkId)" v-has="'project:item:edit'">编辑</el-button>
            <el-button size="mini" plain type="danger" @click="handleDel(row.pkId)" v-has="'project:item:delete'">删除</el-button>
          </template>
        </el-table-column>
      ...
      <div class="dialog-form">
        <el-form :model="formDate" :rules="rules" ref="dialog">
          <!-- <el-form-item label="所属项目" label-width="80px" v-if="user.orgType !== 5">
            <el-select v-model=" formDate.proName" style="width:100%" placeholder="想选择所属项目" size="mini" :disabled="dialogType === 2">
              <el-option  :value="item.pkId" v-for="item in options" :key="item.pkId" :label="item.proName"></el-option>
            </el-select>
          </el-form-item> -->
          ...
          <el-form-item label="施工方案" label-width="80px" prop="projectScheme">
            <el-input size="mini" show-word-limit v-model="formDate.projectScheme" type="textarea" resize="none" :disabled="dialogType === 2" :maxlength="500" rows="4" style="padding-bottom: 20px "></el-input>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 获取工程项目表
    getEnginnerList(projectName) {
      let data = { ...this.page };
      if (projectName) {
        data = { ...data, projectName };
      }
      if (this.fkProjectId) {
        data = { ...data, fkProjectId: this.fkProjectId };
      }
      api.getProjectTableListByCondition(data).then(res => {
        if (res.code === 200) {
          const { data } = res;
    ...
    handleOpen(type, pkId) {
      ...
      if (pkId) {
        api.getProjectTableInfoById({ pkId }).then(res => {
    ...
    async btnOk() {
      ...
      const promise = this.formDate.pkId ? api.editProjectTableById : api.addProjectTable;
      this.loading = true;
    ...
    handleDel(pkId) {
      this.$confirm("确定删除该工程项目信息？", "删除确认", {
        ...
        api.delProjectTableById({ pkId }).then(res => {
          if (res.code === 200) {
            this.getEnginnerList();
