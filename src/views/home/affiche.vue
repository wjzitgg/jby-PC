<template>
  <div class="app-container" v-loading="loading">
    ...
    <el-tabs v-model="radio" @tab-click="radioChange">
      <el-tab-pane  label="未读公告" name="0"></el-tab-pane>
      <el-tab-pane  label="已读公告" name="1"></el-tab-pane>
      <el-tab-pane  label="我发布的" name="2"></el-tab-pane>
    </el-tabs>
    </div>
    <!-- 表格 -->
    <div class="table-data"  style="height:83%">
    <el-table height="100%" :data="tableData" :header-cell-style="{ background: '#f1f6ff' }" stripe :tooltip-effect="'light'">
      ...
      <el-table-column label="标题">
        <template slot-scope="{ row }">
          ...
          <span style="color:#02a7f0;cursor: pointer;" class="ellipsistwo" @click="openInfoDialog(row)">{{ radio==2?row.title:row.noticeTitle }}</span>
          </div>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      let data = { ...this.page, readStatus: (this.radio -0), noticeType: 1 };
      if(this.radio==0){
