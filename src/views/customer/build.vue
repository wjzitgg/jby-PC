  ...
  <div class="app-container">
    ...
    <div slot="header" class="clearfix card-header search-bar-box">
      <div class="head">
        ...
        <el-button size="mini" type="primary" class="addBtn el-button-primary" @click="handleOpen(3)" v-if="user.orgType === 5">新增</el-button>
      </div>
    </div>
    <div class="tab-container">
      <div class="table-data" style="height: 86%;">
        <!-- 表格 -->
        <el-table ref="tables" stripe :data="tableData" height="90%" :header-cell-style="{ background: '#f1f6ff' }" :tooltip-effect="'light'">
          ...
          <el-table-column label="建设单位名称" prop="customName" align="center"> </el-table-column>
          ...
          <el-table-column label="绑定关联" width="100" align="center" v-if="user.orgType !== 3">
            <template slot-scope="{ row }">
              <span>{{ row.relationStatus === 0 ? "否" : "是" }}</span>
            </template>
          </el-table-column>
          <el-table-column label="备注" prop="remark" width="300px" align="center" v-if="user.orgType == 5" show-overflow-tooltip> </el-table-column>
          <el-table-column label="项目名称" prop="fkProjectName" align="center" v-if="user.orgType === 3" show-overflow-tooltip> </el-table-column>
          ...
          <el-table-column label="操作" align="center" width="350" v-if="user.orgType === 5">
            <template slot-scope="{ row }">
              ...
              <el-button plain size="mini" type="primary" @click="openLinkDialog(row)" v-show="row.relationStatus - 0 != 1 && user.orgType === 5">绑定关联</el-button>
    ...
    <!-- 模态框 -->
    <el-dialog :close-on-press-escape="false" :title="(dialogType === 2 ? '查看' : dialogType === 1 ? '编辑' : '新增') + '建设单位'" :visible.sync="dialogVisible" @close="handleCancel" ref="model" width="25%" v-loading="loading" v-dialogDrag :close-on-click-modal="false">
      <el-form :model="formDate" class="dialog-form" label-width="90px" :rules="rules" ref="formDate">
        <!-- <el-form-item label="关联项目" v-if="user.orgType === 3">
          <el-input
            size="mini"
            v-if="dialogType !== 3"
            disabled
            :value="formDate.fkProjectName"
          ></el-input>
          <el-select v-model="formDate.fkProjectId" size="mini" v-else>
            <el-option
        ...
        <el-form-item label="联系人" prop="linkMan">
          <el-input size="mini" v-model="formDate.linkMan" :disabled="dialogType === 2" :maxlength="25"></el-input>
        </el-form-item>
        <el-form-item label="联系电话" prop="linkPhone">
          <el-input size="mini" v-model.trim="formDate.linkPhone" :disabled="dialogType === 2" type="number" oninput="if(value.length>11)value=value.slice(0, 11)"></el-input>
        </el-form-item>
        <el-form-item label="备注" v-if="user.orgType !== 3">
          <el-input size="mini" v-model="formDate.remark" maxlength="100" type="textarea" resize="none" :disabled="dialogType === 2" rows="8"></el-input>
        </el-form-item>
      </el-form>
      <div style="height: 10px;" v-if="dialogType == 2"></div>
      <div slot="footer" class="dialog-footer" v-if="dialogType !== 2">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        customName: [{ required: true, message: "请填写建设单位名称", trigger: "blur" }],
  ...
  methods: {
    ...
    // 获取建设单位列表
    getCapitalList() {
      const data = { ...this.page, customType: 0, ...this.searchData };
    ...
    removeBind(row) {
      ...
      // this.$confirm("确定解除该客户信息与建设单位在系统中的关联关系？", "解除关联确认", {
    ...
    querySearch(queryString, cb) {
      if (/^1(3|4|5|6|7|8|9)\d{9}$/.test(queryString)) {
        let date = { linkPhone: queryString, orgType: 2 };
