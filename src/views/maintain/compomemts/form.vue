<template>
  <div>
    <div class="search-bgc search-mr">
      <el-form :model="searchData" class="form-data">
        ...
        <el-form-item label="表格名称" label-width="90px">
          <el-input v-model="searchData.tableName" size="mini" class="wh-200"></el-input>
        </el-form-item>
        <el-button size="mini" type="primary" class="seek" @click="search(searchData)">搜索</el-button>
        <el-button type="primary" size="mini" v-has="'maintain:customTable:add'" class="add-staff btn-right" @click="addUnit">新建表格</el-button>
      </el-form>
    </div>
    <div class="tab-container">
      <el-table :data="tableData" border ref="tables" height="68%" style="width: 100%" v-loading="loading" :header-cell-style="{ background: '#f1f6ff' }" :cell-style="{ align: 'center' }" :tooltip-effect="'light'">
        ...
        <el-table-column align="center" prop="enginerringTypeName" label="工程大类" min-width="120" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="professionalTypeName" label="专业类别" min-width="120" show-overflow-tooltip></el-table-column>
        <el-table-column align="center" prop="tableName" label="表格名称" min-width="120">
          <template slot-scope="{row}">
            <div class="ellipsistwo">{{row.tableName}}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="alarmLevel" label="操作" min-width="220">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click.native="examine(scope.row)">预览</el-button>
            <el-button size="mini" @click.native="edit(scope.row)" v-has="'maintain:customTable:update'">编辑</el-button>
            <el-button type="danger" size="mini" v-has="'maintain:customTable:delete'" @click.native="remove(scope.row)">删除</el-button>
            <el-button type="primary" size="mini" @click.native="copyAdd(scope.row)">复制新增</el-button>
          </template>
        </el-table-column>
      ...
      <div style="position: relative">
        <el-form :model="form" :rules="rules" ref="form" style="margin-top: 20px">
          <el-form-item label="工程大类" prop="fkEnginerringTypeId" :label-width="formLabelWidth" v-if="headDate.orgType == 5">
            <el-select style="width: 100%" disabled v-model="form.fkEnginerringTypeId" placeholder="请选择">
          ...
          <el-form-item label="专业类别" prop="fkProfessionalTypeId" :label-width="formLabelWidth" v-if="headDate.orgType == 5">
            <el-select style="width: 100%" :disabled="showType == 4" v-model="form.fkProfessionalTypeId" placeholder="请选择">
          ...
          <el-form-item label="表格类型" prop="tableType" :label-width="formLabelWidth">
            <el-select style="width: 100%" :disabled="showType == 4 || headDate.orgType != '5'" v-model="form.tableType" placeholder="请选择">
          ...
          <el-form-item label="排序值" prop="sortval" :label-width="formLabelWidth">
            <el-input v-model="form.sortval" autocomplete="off" type="number" oninput="if(value.length > 6) value=value.slice(0, 6)" :disabled="showType == 4"></el-input>
          </el-form-item>
    ...
    <el-dialog title="预览表格" :visible.sync="show2" :width="pvwWidth" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false">
      <div style="padding: 8px;text-align: right;"><el-button @click="print" type="primary" size="mini">打印</el-button></div>
      <div :style="'width:' + pvwWidth + 'px'" style="margin: 0 auto;" v-if="show2">
        <div style="width:100%;height:500px;overflow: auto;" ref="previewTable">
          <table style="border-collapse: collapse;overflow: auto;margin: 0 auto;" ref="printPage" id="printPagesa">
            <tr v-for="(item, idx) in tabulationArr" :key="idx" style="">
              <td style="box-sizing: content-box;border:none;margin:0;padding:0" :class="{ 'pre-wrap': i.name.indexOf('\n') != -1 }" @blur="tdBlur(item, id, $event.target.innerText, $event)" v-for="(i, id) in item.arr" :style="[i.style]" :key="id" :rowspan="i.row" :colspan="i.span">
                <div v-if="i.attributeTypes == 1 && i.checkout == false" :class="{ 'pre-wrap': i.name.indexOf('\n') != -1 }">
                  {{ i.name }}
                </div>
                <el-input v-if="i.attributeTypes == 1 && i.checkout == 1" disabled @input="tdNameChange(item, id, i.name)" @blur="tdNameBlur(item, id, i.name)" v-model="i.name" size="mini"></el-input>
    ...
    <el-dialog class="custom-table-edit" id="customTableEdit" ref="customTableEdit" width="80%" height="100%" title="设计表格" :visible.sync="show3" v-dialogDrag :close-on-click-modal="false" :show-close="false" :close-on-press-escape="false" v-loading="loading" custom-class="design-table-edit">
      <div class="operate">
        ...
        <div style="margin-left: 12px;width: 34px;">
          纸张属性
        </div>
        <div style="line-height: 34px;width: 34px;text-align: center;" class="pitch-on">
          A4
        </div>
        <div style="margin-left: 12px;width: 4px;height: 36px;background: #ccc;"></div>
        <div style="margin-left: 12px;width: 34px;">
        ...
        <div class="short-dmOrientation" @click="vertical" style="line-height: 34px;width: 34px;text-align: center;background: #ccc;" :class="{ 'pitch-on': shortDmOrientation == 1 }">
          纵向
        </div>
        <div class="short-dmOrientation" @click="crosswise" style=" margin-left: 12px;line-height: 34px;width: 34px;text-align: center;background: #ccc;" :class="{ 'pitch-on': shortDmOrientation == 2 }">
          横向
        </div>
        <div style="margin-left: 12px;width: 4px;height: 36px;background: #ccc;"></div>
        <div style="margin-left: 12px;width: 34px;">
      ...
      <div class="custom-table" @click="(rightEditStatus = false), (copyStatus = false)">
        <div class="left leftTable" style="position: relative;" id="leftTable" ref="leftTable">
          <div style="text-align: left;">
            <div ref="colWidthBox" style="display:inline-flex; position: relative;text-align: center;background: #ccc;" v-if="tabulation.length != 0">
              ...
              <div ref="colWidth" :id="'colWidth' + idx" class="colWidth" @mousedown="widthClick(idx, $event)" @mouseup="wdMouseUp(idx, $event)" @contextmenu.stop.prevent="heightRightClick(idx, $event, 2)" style="width:50px;border: 1px solid;" v-for="(item, idx) in tabulation[tabulation.length - 1].arr" :key="idx">{{ idx + 1 }}</div>
            ...
            <div style="position: relative;display: flex;text-align: center;" class="table-box" ref="tableBoxY" v-if="show3" @scroll="handleScroll">
              <div v-if="tabulation.length != 0 && show3" style="display: inline-block;position: relative;z-index: 999;" ref="rowHeightBox">
                <div ref="rowHeight" :id="'rowHeight' + idx" class="rowHeight" @mousedown="heightClick(idx, $event)" @contextmenu.stop.prevent="heightRightClick(idx, $event, 1)" style="width:50px;border: 1px solid;height: 30px;line-height: 34px;background: #ccc;" v-for="(item, idx) in tabulation" :key="idx">{{ idx + 1 }}</div>
              </div>
              <div style=" position: relative;">
                <table class="table mainTable" ref="tableDesign" v-if="refreshStatus">
                  <tr v-for="(item, idx) in tabulation" :key="idx" class="table-tr" ref="tableTr">
                    <td v-for="(i, id) in item.arr" :style="[i.style]" :key="id" ref="tableTd" @blur="nameInput(idx, id, $event.target.innerText, $event, i)"  :class="{ 'pre-wrap': i.name.indexOf('\n') != -1 }"
                     @input="tdInput(idx, id, $event.target.innerText, $event, i)" :contentEditable="i.attributeTypes == 1" class="table-td" :rowspan="i.row" :colspan="i.span" style=" border: 1px solid rgba(0,0,0,0.2); -webkit-touch-callout: none;" @mousedown="onmousedownClick(idx, id, $event)" @contextmenu.stop.prevent="tdRightClick(idx, $event)">
              ...
              border-radius: 6px;z-index: 9999;"
                >
                  <div
                    class="arrow"
                    style="border-bottom: 7px solid rgba(0,0,0,0);
        ...
        <div class="right">
          <div style="background: #ccc; line-height: 22px;margin-bottom: 8px;text-align: center;">
            选择区域
          </div>
          <div class="right-row-span">
            <span>行</span>
          ...
          <div style="margin-top:10px;background: #ccc; line-height: 22px;margin-bottom: 8px;text-align: center;">
            单元格属性
          </div>
          <div class="right-row-span" v-if="row.length != 0 && span.length != 0">
            <span>类型</span>
          ...
          <div class="right-row-span" v-if="attributeTypes == 1">
          ...
          <div class="right-row-span" v-if="attributeTypes == 2" style="height: 220px; overflow: auto;">
          ...
          <div class="right-row-span" v-if="attributeTypes == 3" style="height: 220px; overflow: auto;">
            <div>
              <span>内容</span>
...
export default {
  ...
  methods: {
    ...
    searchSysAuditRole() {
      this.$api.searchSysAuditRole({ type: 9 }).then(res => {
        if (res.code == 200) {
    ...
    // 打印
    print() {
      const printContent = this.$refs.printPage;
      // 获取dom 宽度 高度
            ...
            if (idx == this.rightEditIdx) {
              if (item.style.display == "none") {
                if (item.mergeSpan != undefined && item.mergeSpan != idx) {
                  ...
                  this.tabulation[index].arr[item.mergeSpan].style.width = Number(wd) - Number(w) + "px";
          ...
          if (ind.style.display == "none") {
            if (ind.mergeSpan != undefined && ind.mergeSpan != this.rightEditIdx) {
              ...
              if (ind.mergeRow == undefined || ind.mergeRow == index) {
                ...
                this.tabulation[index].arr[ind.mergeSpan].style.width = Number(w) + 50 + "px";
    ...
    widthFunction() {
      const left = this.shortDmOrientation == 1 ? 605 : 860;
      let num = 0;
      let status = true;
      let wd = 0;
      this.$refs.colWidth.forEach((item, i) => {
      ...
      const top = this.shortDmOrientation == 1 ? 860 : 605;
      let h = 0;
      let hStatus = true;
        ...
        if (_this.$refs.rectangular != undefined) {
          ...
          if (_this.start_x < end_x) {
            ...
            if (_this.start_y > end_y) {
              ...
              const tableBottom = document.getElementById("leftTable").getBoundingClientRect().bottom - 50;
              if (end_y - Xd.y > customTable.offsetHeight - 30) {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.showType == 1) {
            ...
            if (this.headDate.orgType != 5) {
              addForm.fkProfessionalTypeId = 0;
              addForm.fkEnginerringTypeId = 0;
            }
