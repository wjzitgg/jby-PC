  ...
  <div class="app-container" v-loading="loading">
    ...
    <div class="header-search">
      <el-form label-width="80px" :inline="true" class="myform">
        <el-form-item label="项目" v-if="user.orgType === 2 || user.orgType === 3 || user.orgType === 9" label-width="60px">
        ...
        <el-form-item label="标段" v-if="user.orgType !== 5" label-width="60px">
          <el-select v-model="inpData.fkBidProjectId" :disabled="!inpData.fkProjectId" size="mini" v-if="user.orgType === 2 || user.orgType === 3 || user.orgType === 9" class="wh-140" @change="selectItem($event)">
            <el-option value="" label="全部"></el-option>
        ...
        <el-form-item label="变更类型">
          <el-select v-model="inpData.fkWorkflowTemplateId" size="mini" class="wh-140">
            <el-option value="" label="全部"></el-option>
            <el-option v-for="item in changeTypeOptins2" :key="item.value" :label="item.label" :value="item.value"> </el-option>
          </el-select>
        </el-form-item>
      ...
      <el-button size="mini" type="primary" class="el-button-primary" v-has="'projectManage:change:add'" v-if="user.orgType !== 4" @click="openAddDialog(1)">发起变更</el-button>
    </div>
    <!-- 表格 -->
    <div class="table-data">
      <el-table ref="tables" :data="tableData" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }" :tooltip-effect="'light'">
        ...
        <el-table-column label="发起日期" prop="launchTime" align="center" show-overflow-tooltip> </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="250">
          <template slot-scope="{ row }">
            ...
            <el-button size="mini" plain v-has="'projectManage:change:query'" type="primary" v-show="![5, 6].includes(row.changesStatus)" @click="openApprovalDialog(1, row)">查看</el-button>
            <el-button size="mini" plain v-has="'projectManage:change:query'" type="primary" v-show="[5, 6].includes(row.changesStatus)" @click="openApprovalDialog(4, row)">查看</el-button>
            <el-button size="mini" plain v-has="'projectManage:change:update'" type="success" v-show="row.editStatus" @click="openAddDialog(2, row)">{{ [3, 7].includes(row.changesStatus) ? "重新发起" : "编辑" }}</el-button>
            <el-button size="mini" plain v-has="'projectManage:change:delete'" type="danger" v-show="row.deleteStatus" @click="handelDel(row)">删除</el-button>
            ...
            <el-button size="mini" plain type="success" v-has="'projectManage:change:download'" v-show="row.downloadStatus" @click="downloadContract(row)">下载合同</el-button>
          </template>
        </el-table-column>
      ...
      <div class="dialog-form">
        ...
        <el-form :inline="true" :model="formData" :rules="rules" ref="addform1" label-width="80px" v-if="user.orgType !== 5">
          ...
          <el-form-item label="标段" prop="fkBidProjectId" v-if="user.orgType === 2 || user.orgType === 3 || user.orgType === 9">
            <el-select v-model="formData.fkBidProjectId" size="mini" :disabled="!formData.fkProjectId" @change="selectItem($event)" class="wh-200">
    ...
    <el-dialog :close-on-press-escape="false" title="设置审批人" width="70%" class="tree-dialog" top="5vh" :visible="approvalShow" v-loading="loading" v-dialogDrag :close-on-click-modal="false" @close="approvalCancel(0)">
      <div class="approver-edit" :style="{ overflow: approvalData.launchType == 2 && !approvalData.workflowNodeDTOS[1].prodSysRoleVo.selectedUserId ? 'hidden' : 'auto' }">
        ...
        <div class="node-box">
          <div v-for="(item, index) in approvalData.workflowNodeDTOS" class="node-item" :key="index" v-show="item.nodeType == 2" style="margin-top:20px">
            <div class="node">
...
import signBox from "../design/component/signBox.vue";
...
export default {
  components: { pdfPiev, pdf, signBox, approvalDialog, chapterProcess, chapterProcessApproval, approveCodeDialog },
  computed: {
    user() {
  ...
  methods: {
    ...
    selectItem(item) {
      ...
      this.formData.fkWorkflowTemplateId = "";
      this.getChangeType(item);
    },
    approveOk(unique) {
      this.btnOk(1, unique);
    },
          ...
          item.tableHtml.forEach(e => {
            e.arr.forEach(i => {
              if (i.attributeTypes == 3) {
                if ([...this.user.roleList, ...this.user.systemRoleList].includes(i.signatureDate.signer)) {
    ...
    // 查询变更类型
    getChangeType(id, isTrue) {
      this.$api.gnInfoGetChangeType({ fkBidProjectId: id }).then(res => {
        if (res.code === 200) {
          if (this.addDialog) {
            this.changeTypeOptins = res.data.map(item => ({ ...item, value: item.key, label: item.name }));
            if (isTrue) {
              const arr = this.changeTypeOptins.filter(item => this.formData.fkWorkflowTemplateId === item.value);
              if (!arr.length) {
                this.approvalStatus = false;
                this.formData.fkWorkflowTemplateId = this.formData.fkWorkflowTemplateName;
                this.dis = true;
                setTimeout(() => {
            ...
            this.changeTypeOptins2 = res.data.map(item => ({ ...item, value: item.key, label: item.name }));
          }
        } else {
          this.$message.warning(res.msg);
        }
    ...
    // 查询变更状态
    changeStatsQuery() {
      this.$api.changesChangeStatsQuery().then(res => {
        if (res.code === 200) {
    ...
    // 分页列表
    searchPageDesign() {
      this.$api.changesDesignInfoSearchPage({ ...this.page, ...this.searchData }).then(res => {
        console.log("分页", res);
        if (res.code === 200) {
    ...
    getDesignDetail(changeId, type, sendType) {
      ...
      // type:1为编辑，2为打开审批弹框
      this.$api
        .changesDesignInfoDetail({ changeId, type: sendType })
        .then(res => {
          console.log("详情", res);
          if (res.code === 200) {
            ...
            if (type === 1) {
              ...
              this.$nextTick(() => {
                this.getChangeType(this.showData.fkBidProjectId, true);
              });
    ...
    handelDel(row) {
      ...
      this.$confirm("确定删除该变更请求?", "提示", {
        ...
        this.$api.changesDesignInfoDelete({ changeId }).then(res => {
          if (res.code === 200) {
            this.btnSearch();
