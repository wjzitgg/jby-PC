  ...
  <div class="app-container">
      ...
      <template slot="left">
        <el-form ref="form" label-width="80px">
          ...
          <el-form-item label="结算标段" v-if="user.orgType !== 5">
            <el-select v-model="linkProId" size="mini" v-if="user.orgType === 2 || user.orgType === 3" class="wh-200" @change="bidProChange($event, 1)">
              <el-option value="" label="全部"></el-option>
            ...
            <el-select v-model="linkProId" size="mini" class="wh-200" @change="bidProChange($event, 2)" v-else>
              <el-option value="" label="全部"></el-option>
              <el-option v-for="item in options" :key="item.pkId" :value="item.pkId" :label="item.projectName"></el-option>
            </el-select>
          </el-form-item>
    ...
    <div class="table-data" style="height:80%">
      <el-table :tooltip-effect="'light'" :data="tableData" ref="tables" height="100%" :show-summary="showSummary" :summary-method="getSummaries" stripe :header-cell-style="{ background: '#f1f6ff' }" :key="tableKey">
        ...
        <el-table-column align="center" label="本期末含税结算金额（元）" prop="tipSettleAmount" show-overflow-tooltip>
          <!-- <template slot-scope="{ row }">
        ...
        <el-table-column align="center" label="操作" width="350">
          <template slot-scope="{ row }">
            <div>
              ...
              <el-button size="mini" plain @click="showDialog(4, row.pkId)" v-has="'projectManage:metering:approveQuery'" v-show="![5, 6].includes(row.settleStatus)">查看审批详情</el-button>
              <el-button size="mini" plain @click="showDialog(7, row.pkId)" v-has="'projectManage:metering:approveQuery'" v-show="[5, 6].includes(row.settleStatus)">查看审批详情</el-button>
...
import MyDialog from "../settlement/components/MyDialog";
import subDialog from "../settlement/components/subDialog";
import SearchBar from "../settlement/components/SearchBar.vue";
...
export default {
  ...
  methods: {
    ...
    btnSearch() {
      ...
      //     orgType: this.sendOrgType
      //   };
      // } else if ((this.proId && !this.linkProId) || (this.proId && !this.linkProId)) {
      //   this.searchData = {
      //     ...this.inpData,
    ...
    bidProChange(e, type) {
      ...
      //   this.inpData = { ...this.inpData, isWholeBid: 1 };
      // }
      // let fkProjectId = this.linkProId.join(",");
      // this.inpData = { ...this.inpData, isWholeBid: 0, fkProjectId };
  ...
  computed: {
    ...
    pricePermission() {
      let arr = this.$store.state.menuData.filter(item => item.menuCode == "projectManage/metering");
