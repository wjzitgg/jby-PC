<template>
  ...
  <div class="app-container">
    ...
    <div class="topTabs">
      <el-tabs v-model="radio" v-if="user.orgType === 4 || user.orgType === 5">
    ...
    <div class="header-seach">
      <el-form :inline="true" class="myform" label-width="72px">
        <el-form-item label="项目" v-if="user.orgType === 2 || user.orgType === 3 || user.orgType === 9">
          <el-select v-model="inpData.fkProjectId" @change="proChange($event, 1)" size="mini" class="w-180">
        ...
        <el-form-item label="标段项目" v-if="user.orgType !== 5">
          ...
          <el-select v-model="inpData.fkBidProjectId" size="mini" class="w-180" @change="fkBidProjectIdChange($event, 1)" v-else>
            <el-option value="" label="全部"></el-option>
            <el-option v-for="item in options" :key="item.pkId" :value="item.pkId" :label="item.projectName"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="交底单位" v-if="([4, 5].includes(this.user.orgType) && radio == 1) || user.orgType === 7">
        ...
        <el-form-item label="交底部门" v-if="([4, 5].includes(this.user.orgType) && radio != 1) || [2, 3, 9].includes(this.user.orgType)">
        ...
        <el-form-item label="接收单位" v-if="([4, 5].includes(this.user.orgType) && radio == 3) || [2, 3, 9].includes(this.user.orgType)">
          <el-select v-model="inpData.fkReceivingOrgId" size="mini" class="w-180" :disabled="!inpData.fkBidProjectId && this.user.orgType !== 5">
            <el-option value="" label="全部"></el-option>
            <el-option v-for="item in receiveUnitList" :key="item.pkId" :value="item.value" :label="item.label"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="接收部门" v-if="[4, 5].includes(this.user.orgType) && radio != 3">
      ...
      <el-button size="mini" type="primary" class="el-button-primary" v-has="'projectManage:disclosure:add'" @click="handleOpen(2)">新增交底</el-button>
    </div>
    <!-- 表格 -->
    <div class="table-data" :style="{ height: ([4, 5].includes(this.user.orgType) && radio == 3) || [2, 3, 9].includes(this.user.orgType) ? '65%' : '73%' }">
      ...
      <el-table-column label="标段" prop="fkBidProjectName" align="center" show-overflow-tooltip v-if="user.orgType !== 5"> </el-table-column> -->
        <el-table-column label="交底名称" prop="disclosureName" align="center"> </el-table-column>
        <el-table-column label="交底部门" prop="fkDeptName" align="center" show-overflow-tooltip v-if="([4, 5].includes(this.user.orgType) && radio != 1) || [2, 3, 9].includes(this.user.orgType)"> </el-table-column>
        <el-table-column label="交底单位" prop="fkOrgName" align="center" show-overflow-tooltip v-if="([4, 5].includes(this.user.orgType) && radio == 1) || user.orgType === 7"> </el-table-column>
        <el-table-column label="交底人" prop="fkUserName" align="center" show-overflow-tooltip> </el-table-column>
        <el-table-column label="接收单位" prop="fkReceivingOrgName" align="center" show-overflow-tooltip v-if="([4, 5].includes(this.user.orgType) && radio == 3) || [2, 3, 9].includes(this.user.orgType)"> </el-table-column>
        <el-table-column label="接收人" prop="fkReceivingUserName" align="center" show-overflow-tooltip> </el-table-column>
        <el-table-column label="接收部门" prop="fkReceivingDeptName" align="center" show-overflow-tooltip v-if="[4, 5].includes(this.user.orgType) && radio != 3"> </el-table-column>
        ...
        <el-table-column label="交底状态" prop="enableStatus" align="center" show-overflow-tooltip v-if="([4, 5].includes(user.orgType) && radio != 1) || [2, 3, 9].includes(user.orgType)">
          <template slot-scope="{ row }">
        ...
        <el-table-column label="操作" align="center" width="250">
          <template slot-scope="{ row }">
            <el-button plain type="primary" @click="handleOpen(1, row)" v-show="([1, 3].includes(row.enableStatus) && user.orgType === 4) || user.orgType !== 4" size="mini">查看</el-button>
            <el-button plain size="mini" type="success" v-show="row.isReceive" @click="accept(row)">接收</el-button>
            <el-button plain size="mini" v-show="row.isCopy" v-has="'projectManage:disclosure:distribute'" @click="openCopyDialog(row)">抄送</el-button>
            <el-button plain size="mini" type="success" v-has="'projectManage:disclosure:update'" v-show="row.editStatus" @click="handleOpen(3, row)">编辑</el-button>
            <el-button plain size="mini" type="danger" v-has="'projectManage:disclosure:delete'" v-show="row.deleteStatus" @click="handleDel(row)">删除</el-button>
            <el-button plain size="mini" type="danger" v-has="'projectManage:disclosure:withdraw'" v-show="row.isWithdraw" @click="recall(row)">撤回</el-button>
    ...
    <el-dialog :title="title" top="1vw" center :visible="dialogVisible" width="30%" @close="handleClose(false)" :close-on-press-escape="false" v-loading="loading" v-dialogDrag :close-on-click-modal="false">
      <div style="position: relative;" class="dialog-form add-item">
    ...
    <el-dialog title="查看交底" center top="5vw" width="80%" :visible="show" @close="checkDialogClose" :close-on-press-escape="false" v-loading="loading" v-dialogDrag :close-on-click-modal="false">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="withdraw" v-if="activeName == 'second'">撤 回</el-button>
...
export default {
  components: { tablePaging, pdfPiev },
  computed: {
    user() {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        // disclosureTime: [{ required: true, message: "请输入交底时间", trigger: "blur" }],
        fkDeptId: [{ required: true, message: "请选择交底部门", trigger: "blur" }],
        ...
        fkUserId: [
          { required: true, message: "请选择交底人", trigger: "blur" },
  ...
  methods: {
    withdraw() {
      ...
      this.$confirm("确定撤回抄送记录?", "提示", {
        ...
        this.$api.projectTechnicalRetractCopy({ copyIds: arr }).then(res => {
          if (res.code === 200) {
    ...
    searchDisclosurePage() {
      let disclosureType = "";
      if (this.user.orgType === 5 || this.user.orgType === 4) {
      ...
      this.$api.projectTechnicalDisclosure({ ...this.page, disclosureType, ...this.searchData }).then(res => {
        console.log("分页", res);
        if (res.code === 200) {
    ...
    searchByCompanies() {
      this.$api.searchByCompanies().then(res => {
        ...
        if (res.code === 200) {
          this.disclosureUnitList = res.data.map(item => ({ ...item, value: item.pkId, label: item.customName }));
        } else {
          this.$message.warning(res.msg);
        }
      });
    ...
    async btnAdd(enableStatus) {
      ...
      let data = { ...this.formDate, enclosures: this.fileList, enableStatus };
      this.loading = true;
      if (this.dialogType === 2) {
        this.$api
          .projectTechnicalDisclosureAdd(data)
          .then(res => {
            console.log("新增", res);
            this.loading = false;
            if (res.code === 200) {
        ...
        // 编辑
        this.$api
          .projectTechnicalDisclosureUpdate(data)
          .then(res => {
            console.log("编辑", res);
            this.loading = false;
            if (res.code === 200) {
    ...
    // 选择交底部门后的change事件
    fkDeptIdChange(e) {
      let arr = this.disclosureDepList.filter(item => item.pkId === e)[0].users ? this.disclosureDepList.filter(item => item.pkId === e)[0].users : [];
      this.disclosurePeopleList = arr.map(item => ({ ...item, value: item.pkId, label: item.userName }));
    },
    // 选择接收单位后的change事件
    fkReceivingOrgIdChange(e) {
      if (this.user.orgType === 5) {
        let arr = this.subList.filter(item => item.fkLinkOrgId === e)[0].users ? this.subList.filter(item => item.fkLinkOrgId === e)[0].users : [];
        ...
        let arr = this.addreceiveUnitList.filter(item => item.fkOrgId === e).length ? this.addreceiveUnitList.filter(item => item.fkOrgId === e)[0].deptVoLists.map(item => ({ ...item, value: item.pkId, label: item.deptName })) : [];
    ...
    fkReceivingDeptIdChange(e) {
       this.formDate.fkReceivingUserId = "";
      if (this.user.orgType === 5) {
        let arr = this.receiveDepList.filter(item => item.pkId === e)[0].users ? this.receiveDepList.filter(item => item.pkId === e)[0].users : [];
        this.receivePeopleList = arr.map(item => ({ ...item, value: item.pkId, label: item.userName }));
      } else {
        this.formDate.fkReceivingUserId = "";
        let arr = this.receiveDepList.filter(item => item.pkId === e).length ? this.receiveDepList.filter(item => item.pkId === e)[0].users.map(item => ({ ...item, value: item.pkId, label: item.userName })) : [];
        this.receivePeopleList = arr;
      }
    },
    // 标段change事件
    fkBidProjectIdChange(e, type) {
      let arr = this.linkProList.filter(item => item.pkId === e).length ? this.linkProList.filter(item => item.pkId === e) : [];
      if (type === 1) {
    ...
    handleDel(row) {
      this.$confirm("确定删除该交底?", "提示", {
        ...
        this.$api.projectTechnicalDisclosureDelete({ pkId: row.pkId }).then(res => {
          if (res.code === 200) {
            this.searchDisclosurePage();
    ...
    updateEnableStatus(pkId, enableStatus) {
      this.$api.projectTechnicalDisclosurePost({ pkId, enableStatus }).then(res => {
        if (res.code === 200) {
          this.searchDisclosurePage();
    ...
    selectDisclosureById(pkId) {
      this.$api.projectTechnicalDisclosureFindById({ pkId }).then(res => {
        if (res.code === 200) {
          ...
          if (this.dialogType !== 1) {
            if (this.user.orgType === 5) {
              ...
              let arr = this.linkProList.filter(item => item.pkId === res.data.fkBidProjectId).length ? this.linkProList.filter(item => item.pkId === res.data.fkBidProjectId) : [];
              this.addreceiveUnitList = arr.map(item => ({ ...item, value: item.fkOrgId, label: item.projectOrgName }));
    ...
    deptListById(pkId) {
      this.$api.deptListById({ pkId }).then(res => {
        if (res.code === 200) {
          ...
          this.$nextTick(() => {
            let ids = arr.length ? arr.filter(item => item.isSelect).map(item => item.pkId) : [];
    ...
    copyOk() {
      ...
      let userIds = nodes.filter(item => item.isSelect !== 1).map(item => item.pkId);
      if (!userIds.length) {
        return this.$message.warning("未选择抄送人");
      }
      this.$api.projectTechnicalDisclosureCopy({ pkId: this.copyPkId, userIds }).then(res => {
        console.log(res);
        if (res.code === 200) {
