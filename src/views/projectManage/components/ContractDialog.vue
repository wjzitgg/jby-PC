<template>
  <div>
    <!-- 模态框 -->
    <el-dialog :close-on-press-escape="false" top="30px" :title="dialogShowType == 1 ? '合同信息' : dialogShowType == 2 ? '新增合同' : '编辑合同'" :visible="dialogVisible" @close="handleClose(false)" width="70%" v-loading="loading" v-dialogDrag :close-on-click-modal="false">
      <div class="dialog-form">
        <div class="baseInfo">
          <div class="header">
            基本信息
            <div class="print-export" v-if="dialogShowType == 1">
              <!-- v-has="'production:process:out'"
v-has="'production:process:put'" -->
              <el-button type="primary" v-has="'projectManage:contract:out'" class="el-button-primary" size="mini" @click="dataExport">导出</el-button>
              <el-button type="primary" v-has="'projectManage:contract:put'" class="el-button-primary" size="mini" @click="dataPrinting">打印</el-button>
            </div>
          </div>
          <div class="main">
            <div class="main-model">
              <div class="main-items">
                ...
                <el-input size="mini" :class="{ 'wh-800': this.id == 2, 'wh-700': this.id != 2 }" :disabled="dialogShowType === 1" v-model="addContractDate.contractName" maxlength="50"></el-input>
              </div>
              <div class="main-items">
                <span class="labe-title ml-50">合同编号</span>
                <el-input size="mini" onkeyup="value=value.replace(/[\u4E00-\u9FA5]/g,'')" :class="{ 'wh-200': this.id == 2, 'wh-250': this.id != 2 }" :disabled="dialogShowType === 1 || allowModifyStatus" v-model="contractNumber" maxlength="50"></el-input>
              </div>
            </div>
            <div class="main-model">
              <!-- <div class="main-item">
              ...
              <div class="main-item" v-if="id == 2">
                ...
                <el-input size="mini" :class="{ 'wh-200': this.id == 2, 'wh-250': this.id != 2 }" v-if="dialogShowType === 1 || (dialogShowType === 3 && user.orgType === 3)" :value="user.orgType === 3 ? addContractDate.projectName : addContractDate.supervisionName" disabled></el-input>
                <el-select size="mini" :class="{ 'wh-200': this.id == 2, 'wh-250': this.id != 2 }" v-else :disabled="user.orgType === 5 && id === 1" v-model="addContractDate.fkProjectId" @change="contraChange">
              ...
              <div class="main-item" v-else>
                <span class="labe-title">{{ contracter }}</span>
                <el-input size="mini" :class="{ 'wh-150': this.id == 2, 'wh-250': this.id != 2 }" v-if="dialogShowType === 1 || (dialogShowType === 3 && user.orgType === 3)" v-model="addContractDate.customerOrgName" disabled></el-input>
                <el-select size="mini" :class="{ 'wh-150': this.id == 2, 'wh-250': this.id != 2 }" v-else :disabled="user.orgType === 5 && id === 1" v-model="addContractDate.fkCustomerId" @change="contraChange">
              ...
              <div class="main-item" v-show="id === 2">
                ...
                <el-input size="mini" :class="{ 'wh-200': this.id == 2, 'wh-250': this.id != 2 }" disabled v-model="addContractDate.customerOrgName"></el-input>
              </div>
              <div class="main-item" v-show="id === 2">
                <span class="labe-title">施工单位</span>
                <el-input size="mini" :class="{ 'wh-200': this.id == 2, 'wh-250': this.id != 2 }" disabled v-model="addContractDate.superiorOrgName"></el-input>
              </div>
              <div class="main-item" v-if="pricePermission && id !== 2">
                <span class="labe-title">合同金额</span>
                <el-input size="mini" :class="{ 'wh-150': this.id == 2, 'wh-250': this.id != 2 }" disabled v-model="contentAmount"></el-input>
                <span>元</span>
              </div>
              <div class="main-item">
                <span class="labe-title">合同签订日期</span>
                <el-date-picker value-format="yyyy-MM-dd" :class="{ 'wh-200': this.id == 2, 'wh-250': this.id != 2 }" size="mini" type="date" :disabled="dialogShowType === 1" v-model="addContractDate.signDate"> </el-date-picker>
              </div>
            </div>
            <div class="main-model">
              <div class="main-item" v-if="pricePermission && id === 2">
                <span class="labe-title">合同金额</span>
                <el-input size="mini" :class="{ 'wh-173': this.id == 2, 'wh-250': this.id != 2 }" disabled v-model="contentAmount"></el-input>
                <span>元</span>
              </div>

              <div class="main-item" v-if="id !== 3">
                <span class="labe-title">施工开始日期</span>
                <el-date-picker value-format="yyyy-MM-dd" :class="{ 'wh-200': this.id == 2, 'wh-250': this.id != 2 }" size="mini" type="date" :disabled="dialogShowType === 1" v-model="addContractDate.beginDate"> </el-date-picker>
              </div>
              <div class="main-item" v-if="id !== 3">
                <span class="labe-title">施工完成日期</span>
                <el-date-picker value-format="yyyy-MM-dd" :class="{ 'wh-200': this.id == 2, 'wh-250': this.id != 2 }" size="mini" type="date" :disabled="dialogShowType === 1" v-model="addContractDate.endDate"> </el-date-picker>
              </div>
              <div class="main-item" v-if="id !== 3 && id !== 2">
                <span class="labe-title">结算税点</span>
                <el-input size="mini" class="wh-80" :disabled="dialogShowType === 1" v-model.trim="addContractDate.taxRate" oninput="value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" @blur="addContractDate.taxRate = $event.target.value"></el-input>
                <span>%</span>
              </div>
              <div class="main-item" v-if="id === 2">
                <span class="labe-title">结算税点</span>
            ...
            <div class="main-model" v-if="(attachments.length && dialogShowType === 1) || dialogShowType !== 1">
              <div class="main-item">
                ...
                <span v-show="dialogShowType != 1"><input type="file" id="upload" ref="file" style="width: 170px" accept="application/pdf" @change="upLoad" /></span>
                <!-- attachName:filename,attachUrl:path,suffix:typeName -->
                <span v-for="(item, idx) in attachments" :key="idx" class="suee-text">
                  <i class="el-icon-link"></i>
                  <!-- 主合同文件 -->
                  <span @click="preview(item.attachUrl)" style="cursor: pointer">{{ item.attachName }}</span>
        ...
        <div class="listInfo" v-if="[1, 2].includes(id)">
          ...
          <el-tabs v-model="tabs" type="card" :before-leave="beforeLeave" :closable="dialogShowType === 2 || dialogShowType === 3" @tab-remove="removeTab" style="position: relative">
            <el-tab-pane :name="item.chapterName" :label="item.chapterName" v-for="(item, index) in tabsList" :key="index">
              <span slot="label" @dblclick="dblclick(item, index)">{{ item.chapterName }}</span>
              <div class="main">
                <el-table stripe :header-cell-style="{ background: '#f1f6ff' }" :data="item.details" style="width: 100%" height="100%" :key="randeKeys">
                  ...
                  <el-table-column align="center" label="合价" prop="amount" v-if="pricePermission" show-overflow-tooltip> </el-table-column>
        ...
        <div class="listInfo" v-if="id === 3">
          ...
          <div class="main">
            ...
            <el-table stripe :header-cell-style="{ background: '#f1f6ff' }" :data="materialList" style="width: 100%" height="100%" :key="randeKeys">
              ...
              <el-table-column align="center" label="物料名称" prop="detailName" show-overflow-tooltip>
                <template slot-scope="{ row }">
                  ...
                  <div class="ellipsistwo" :title="row.detailName" v-else>{{ row.detailName }}</div>
                </template>
              </el-table-column>
            ...
            <el-tab-pane v-for="item in tabs2List" :key="item.value" :name="item.value" :label="item.label">
              <div class="main">
                <el-table border :data="materialShowList" style="width: 100%" height="100%" :key="randeKeys">
                  ...
                  <el-table-column align="center" label="操作" width="100" v-if="dialogShowType !== 1" show-overflow-tooltip>
                    <template slot-scope="{ row, $index }">
      ...
      <div class="dialog-form" v-if="dopeSheet.inventoryCode != 'inventory_itemize'">
        <div class="listDialog">
          ...
          <div class="listDialog-right" v-if="([1, 2].includes(id) && this.dopeSheet.inventoryCode !== 'inventory_cost') || (id === 4 && radio1 === 1 && this.dopeSheet.inventoryCode !== 'inventory_cost')">
            <div class="header flex" v-if="this.dopeSheet.inventoryCode != 'inventory_cost'">
              ...
              <div v-if="quanxian == 1">关联分项清单细目</div>
    ...
    <el-dialog :visible.sync="changeItemCodeDialog" title="" width="40%" top="45vh" v-dialogDrag :close-on-click-modal="false" class="amendDialog">
      ...
      <div class="itemCode">
        ...
        <div class="itemCode-content wh-60per">
          <el-input size="mini" class="wh-60" v-if="changeItemCodeDialogType > 0" @keyup.enter.native="changeInputEnter(1)" v-model="itemCodes[0]" ref="changeInput" oninput="this.value = this.value.replace(/[^0-9]/g, '');" @blur="itemCodes[0] = $event.target.value"></el-input>
          <div class="str" v-if="changeItemCodeDialogType > 1">-</div>
          <el-input size="mini" class="wh-60" v-if="changeItemCodeDialogType > 1" @keyup.enter.native="changeInputEnter(2)" v-model="itemCodes[1]" ref="changeInput2" oninput="this.value = this.value.replace(/[^0-9]/g, '');" @blur="itemCodes[1] = $event.target.value"></el-input>
          <div class="str" v-if="changeItemCodeDialogType > 2">-</div>
          <el-input size="mini" class="wh-60" v-if="changeItemCodeDialogType > 2" @keyup.enter.native="changeInputEnter(3)" v-model="itemCodes[2]" ref="changeInput3" oninput="this.value = this.value.replace(/[^0-9]/g, '');" @blur="itemCodes[2] = $event.target.value"></el-input>
          <div class="str" v-if="changeItemCodeDialogType > 3">-</div>
          <el-input size="mini" class="wh-60" v-if="changeItemCodeDialogType > 3" @keyup.enter.native="changeInputEnter(4)" v-model="itemCodes[3]" ref="changeInput4" oninput="this.value = this.value.replace(/[^0-9]/g, '');" @blur="itemCodes[3] = $event.target.value"></el-input>
          <div class="str" v-if="changeItemCodeDialogType === 5">-</div>
          <el-input size="mini" class="wh-60" v-if="changeItemCodeDialogType === 5" @keyup.enter.native="changeItemCode" v-model="itemCodes[4]" ref="changeInput5" oninput="this.value = this.value.replace(/[^0-9]/g, '');" @blur="itemCodes[4] = $event.target.value"></el-input>
        </div>
          ...
          <div class="listDialog-left">
            <el-form label-width="100px">
              ...
              <el-form-item label="章节名称">
                <el-input v-model.trim="chapterData.name" size="mini" maxlength="20"></el-input>
              </el-form-item>
            </el-form>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    pricePermission() {
      let arr = this.$store.state.menuData.filter(item => item.menuCode == "projectManage/contract");
  ...
  methods: {
    ...
    searchMajorTypeList(type, item) {
      ...
      this.$api.contractInfoMajorTypeList(data).then(res => {
        if (res.code == 200) {
    ...
    initialization(data) {
      ...
      this.$api.findContractById({ pkId: data.pkId }).then(res => {
        if (res.code == 200) {
    ...
    dataExport() {
      this.loading = true;
      this.$api
        .contractInfoBatchExportFile({ contractId: this.rowContractId })
        .then(res => {
          this.loading = false;
          if (res.code === 200) {
    ...
    dataPrinting() {
      this.loading = true;
      this.$api
        .contractPrintExportFile({ contractId: this.rowContractId })
        .then(res => {
          this.loading = false;
          if (res.code === 200) {
