  ...
  <div style="height: 100%">
    ...
    <div slot="header" class="clearfix card-header">
      <div class="head">
        ...
        <el-button size="mini" type="primary" class="addBtn el-button-primary" @click="handleOpen(1)" v-has="'projectManage:info:overviewAdd'">新增项目概况</el-button>
      </div>
    </div>
      ...
      <el-table ref="tables" :data="tableData" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }" :tooltip-effect="'light'">
        ...
        <el-table-column label="操作" align="center" width="400">
          <template slot-scope="{ row }">
            <el-button size="mini" plain type="primary" @click="handleOpen(2, row.pkId)" v-has="'projectManage:info:overviewAdd'">查看</el-button>
            <el-button size="mini" plain type="success" @click="handleOpen(1, row.pkId)" v-has="'projectManage:info:overviewEdit'">编辑</el-button>
            <el-button size="mini" plain type="danger" @click="handleDel(row.pkId)" v-has="'projectManage:info:overviewDelete'">删除</el-button>
            <el-button size="mini" plain @click="openLinkDialog(row.pkId)" v-has="'projectManage:info:overviewSection'">关联标段项目</el-button>
          </template>
        </el-table-column>
      ...
      <div class="dialog-form">
        <el-form :model="formData" label-width="120px" :rules="rules" ref="formData">
          <el-form-item label="项目名称：" prop="projectName">
            <el-input size="mini" v-model.trim="formData.projectName" :disabled="dialogType === 2" :minlength="2" :maxlength="50" class="wh-400" @input="verify"> </el-input>
          </el-form-item>
          <el-form-item label="项目地点：" class="elect" v-if="dialogType === 1" prop="areaName">
            ...
            <el-select size="mini" class="wh-100" v-model="address.area" @change="areaChange">
          ...
          <el-form-item class="elect" label="项目地点：" v-if="dialogType === 2" prop="areaName">
            <!-- <el-input size="mini" class="wh-400" v-model="formData.detailAddress" disabled></el-input>  <i class="iconfont icondingwei" @click="appear"></i> -->
            <el-select size="mini" class="wh-100" v-model="address.province" @change="provinceChange" :disabled="dialogType === 2">
              <el-option v-for="item in provinceList" :key="item.pkId" :value="item.areaName" :label="item.areaName"></el-option>
            </el-select>
            <el-select size="mini" class="wh-100" v-model="address.city" @change="cityChange" :disabled="dialogType === 2">
              <el-option v-for="item in cityList" :key="item.pkId" :value="item.areaName" :label="item.areaName"></el-option>
            </el-select>
            <el-select size="mini" class="wh-100" v-model="address.area" :disabled="dialogType === 2" @change="areaChange">
              <el-option v-for="item in areaList" :key="item.pkId" :value="item.areaName" :label="item.areaName"></el-option>
            </el-select>
            <el-input size="mini" v-model="address.place" :disabled="dialogType === 2" class="wh-400" maxlength="70"></el-input>
            <i class="iconfont icondingwei" @click="appear"></i>
            ...
            <!-- <div class="bd-map1" v-show="mapStatus && dialogVisible">
                <i class="el-icon-circle-close" @click="appear"></i>
                <bdMap @detailedAddress="detailedAddress" :lat="formData.longitude" :lng="formData.latitude" ref="map" @point="point"></bdMap>
              </div> -->
          </el-form-item>
          <!-- <el-form-item label="建设单位：">
            <el-input size="mini" v-model="formData.investorName" class="wh-400" :disabled="dialogType === 2" :minlength="2" :maxlength="25"></el-input>
          </el-form-item>
          <el-form-item label="监理单位：">
            <el-input size="mini" v-model="formData.supervisorName" class="wh-400" :disabled="dialogType === 2" :minlength="2" :maxlength="25"></el-input>
          </el-form-item>
          <el-form-item label="设计院：">
            <el-input size="mini" v-model="formData.designName" class="wh-400" :disabled="dialogType === 2" :minlength="2" :maxlength="25"></el-input>
          </el-form-item>
          <el-form-item label="施工单位：">
            <el-input size="mini" v-model="formData.contractorName" disabled class="wh-400" :minlength="2" :maxlength="25"></el-input>
          </el-form-item> -->
          <el-form-item label="项目金额：" prop="contractAmount">
            <el-input size="mini" v-model.trim="formData.contractAmount" maxlength="15" :disabled="dialogType === 2" class="wh-400" @keydown.native="moneyKeyDown"></el-input>
          </el-form-item>
          <el-form-item label="工期：" prop="duration">
            <el-input size="mini" v-model.trim="formData.duration" :disabled="dialogType === 2" class="wh-400" maxlength="20"></el-input>
          </el-form-item>
          <el-form-item label="开工日期：" prop="beginTime">
            <el-date-picker size="mini" v-model.trim="formData.beginTime" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="选择日期时间" :disabled="dialogType === 2"> </el-date-picker>
          </el-form-item>
          <el-form-item label="竣工日期：" prop="endTime">
            <el-date-picker size="mini" v-model.trim="formData.endTime" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="选择日期时间" :picker-options="queryOptionsEnd" :disabled="dialogType === 2"> </el-date-picker>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        areaName: [{ required: true, message: "请选择地址", trigger: "blur" }]
      },
      page: {
        pageNum: 1,
        pageSize: 10
  ...
  mounted() {
    if (this.user.orgType !== 5) {
      this.projectInfoProjectSearchPage();
    } else {
      this.getProInfoByProId();
    }
  ...
  methods: {
    ...
    areaChange() {
      this.formData.areaName = this.address.area;
    },
    // 获取项目概况列表
    projectInfoProjectSearchPage(projectName) {
      let data = this.page;
      if (projectName) {
        data = { ...data, projectName };
      }
      api.projectInfoProjectSearchPage(data).then(res => {
        if (res.code === 200) {
          const { data } = res;
    ...
    //  根据项目部id获取项目信息
    getProInfoByProId() {
      api.findProjectByProOrgId({ proOrgId: this.user.fkOrgId }).then(res => {
        if (res.code === 200) {
          if (res.data) {
    ...
    handleOpen(type, pkId) {
      ...
      if (pkId) {
        api.findProjectById({ pkId }).then(res => {
          if (res.code === 200) {
            this.formData = res.data;
            this.address = {
              province: res.data.provinceName, // 省
        ...
        this.$refs.formData.validate().then(res => {
          if (res) {
            ...
            if (this.formData.pkId) {
              promise = api.updateProject;
            } else {
              if (this.user.orgType === 5) {
                promise = api.updateProject;
              } else {
                promise = api.projectInfoAddProject;
              }
            }
    ...
    handleDel(pkId) {
      ...
      this.$confirm("确定删除该工程项目信息？", "删除确认", {
        ...
        api.deleteProject({ pkId }).then(res => {
          if (res.code === 200) {
            console.log(this);
            this.$message({
              message: "删除成功",
    ...
    handleSearch() {
      ...
      this.page.pageNum = 1;
      this.projectInfoProjectSearchPage(this.projectName);
    },
    // pageSize改变
    ...
    detailedAddress(data) {
      ...
      this.formData.areaName = addressComponents.district;
      this.getCityList();
      // this.cityList = this.provinceList.filter(item => this.address.province.indexOf(item.areaName) !== -1)[0].sysCityAreaList;
