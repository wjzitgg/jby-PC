<template>
  <div>
    <el-dialog title="仓库管理" width="60%" :visible="warehouseDialog" @close="closeWarehouseDialog" v-dialogDrag :close-on-click-modal="false" v-loading="loading" :close-on-press-escape="false">
      ...
      <div class="header">
        <div class="search">
          ...
          <el-button size="mini" type="primary" class="ml-20" icon="el-icon-search" @click="btnSearch">查询</el-button>
        </div>
        <el-button type="primary" size="mini" @click="openWareDialog(2)" v-if="user.orgType !== 4&&user.orgType !== 10">新增仓库</el-button>
      </div>
      <div class="tables table-data">
        <el-table style="width: 100%" height="100%" :data="tableData" ref="tables" stripe :header-cell-style="{ background: '#f1f6ff'}">
          ...
          <el-table-column label="操作" align="center" width="300">
            <template slot-scope="{ row }">
              <el-button size="mini" plain type="primary" @click="openWareDialog(1, row)">查看</el-button>
      ...
      <tablePaging :pageSize="page.pageSize" :total="total" :currentPage="page.pageNum" @handleSizeChange="handleSizeChange" @handleCurrentChange="handleCurrentChange" v-if="warehouseDialog"></tablePaging>
      </div>
    </el-dialog>
    <el-dialog center :title="wareDialogTitle" :visible="wareDialog" @close="closeWareDialog" v-dialogDrag :close-on-click-modal="false" v-loading="loading" :close-on-press-escape="false">
      ...
      <el-form ref="wareForm" :model="wareForm" :rules="rules" label-width="80px">
        <el-form-item label="仓库名称">
          <el-input v-model="wareForm.warehouseName" size="mini" maxlength="25" :disabled="wareDialogType === 1"></el-input>
        </el-form-item>
      ...
      <div slot="footer">
        ...
        <el-button type="primary" @click="wareDialogOk" v-if="wareDialogType !== 1">保 存</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    searchWarehousePage() {
      ...
      this.$api.searchWarehousePage(data).then(res => {
    ...
    findWarehouseById(pkId, row) {
      this.$api.findWarehouseById({ pkId }).then(res => {
        if (res.code === 200) {
          ...
          if (this.wareForm.warehouseType === 1&&res.data.provinceName) {
            this.address = {
    ...
    closeWarehouseDialog() {
      this.$emit("update:warehouseDialog", false);
      this.page = {
        pageSize: 10,
        pageNum: 1
      };
    ...
    provinceChange() {
      this.address.city = "";
      this.address.area = "";
      let province = this.provinceList.filter(item => item.areaName.indexOf(this.address.province) !== -1||this.address.province.indexOf(item.areaName) !== -1);
    ...
    cityChange() {
      this.address.area = "";
      let city = this.cityList.filter(item => item.areaName.indexOf(this.address.city) !== -1||this.address.city.indexOf(item.areaName) !== -1);
