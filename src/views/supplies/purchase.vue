  ...
  <div class="app-container">
    ...
    <div class="search-bgc search-mr">
      <el-form :model="searchData" class="form-data">
        ...
        <el-button  v-if="typeShow" type="primary" size="mini" class="add-staff" @click="addRecord" v-has="'supplies:purchase:add'">新增采购计划单</el-button>
      </el-form>
    </div>
    <div class="tab-container" v-if="typeShow">
      <div class="table-data" style="height: 75%">
        <el-table :data="tableData" v-loading="loading" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }" ref="tableBox" :tooltip-effect="'light'">
          ...
          <el-table-column prop="confirmUserName" label="确认人" align="center" v-if="user.orgtype == 5"> </el-table-column>
          <el-table-column prop="confirmTime" label="确认时间" align="center" v-if="user.orgtype == 5"> </el-table-column>
          <el-table-column label="状态" prop="purchaseCodeName" align="center"> </el-table-column>
          <el-table-column prop="remark" label="备注" align="center" show-overflow-tooltip> </el-table-column>
          <el-table-column label="操作" align="center" width="300">
            <template slot-scope="{ row }">
              <el-button size="mini" plain @click="lookOver(row, 1)" v-has="'supplies:purchase:query'">查看</el-button>
              <el-button size="mini" plain type="primary" v-has="'supplies:purchase:viewCode'" @click="shareCode(row.pkId)" v-show="row.isCode">分享二维码</el-button>
              <el-button size="mini" plain type="primary" v-has="'supplies:purchase:update'" @click="editRecord(row)" v-show="row.isUpdate">编辑</el-button>
              <el-button size="mini" plain type="danger" v-has="'supplies:purchase:withdraw'" @click="deleteRecord2(row.pkId)" v-show="row.isWithdraw">撤回</el-button>
              <el-button size="mini" plain type="danger" v-has="'supplies:purchase:delete'" @click="deleteRecord(row.pkId)" v-show="row.isDelete">删除</el-button>
            </template>
          </el-table-column>
      ...
      <div class="dialog-form">
        ...
        <div class="tab-two" v-show="isCollapse == 0">
          <el-form :model="form" :rules="rules" ref="form" class="row-height">
            <el-row :span="24" class="el-row-data">
              ...
              <!-- <el-col :span="8">
                <el-form-item label="负责人" prop="leaderName" label-width="120px">
              ...
              <el-col :span="8" v-if="btnStatus || num == 2">
                <el-form-item label="填表人" label-width="140px">
          ...
          <div class="table-data" :class="num == 0 ? 'table2' : 'table1'">
            <el-table :data="accessoryList" height="100%" style="width: 100%" stripe :header-cell-style="{ background: '#f1f6ff' }" :key="random">
        ...
        <div class="tab-two table-data" v-show="isCollapse == 1">
          <el-table :data="tableBills" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }">
            ...
            <el-table-column prop="receiverTime" label="签收时间" align="center">
              <template slot-scope="{ row }">
    ...
    <materialsSelect :materDialog.sync="materDialog" :selectMater="accessoryList" @getSelect="getSelect" :showType="2" :fkCustomId="form.fkCustomerId" materialTypeCode="materials_common" v-if="num != 1"></materialsSelect>
...
export default {
  ...
  methods: {
              ...
              this.$api.addQRCode().then(res => {
                if (res.code === 200) {
                  ...
                  new QRCode("qrCode", {
                    ...
                    width: 260,
                    height: 260,
                    text: `${location}/h5/#/pages/h5/scanCodeTran?type=5&data=${data}&unique=${this.unique}`
                  });
                  this.queryQRCode();
                } else {
    ...
    // 获取供应商
    supplier(id) {
      this.$api.searchNoLinkOrgId({ projectBidId: id, customType: 3, supplierType: 1 }).then(res => {
        if (res.code == 200) {
          res.data.forEach(item => {
    ...
    // 获取 关联物资申请单
    getApplicationData(orderCode) {
      this.$api.getOrderApply({ pageNum: 1, pageSize: 20, orderCode, fkCustomerId: this.form.fkCustomerId, customerType: 3 }).then(res => {
        if (res.code == 200) {
          res.data.forEach(item => {
            item.label = item.customName;
            item.value = item.pkId;
    ...
    addRecord() {
      ...
      this.accessoryList = [{ fkMaterialId: 1, materialName: "", materialTypeName: "", unitName: "", purchaseNum: "" }]; // 初始化物料
      // 获取申请单号
      this.$api.getBusinessCode({ businessType: 3 }).then(res => {
        if (res.code === 200) {
