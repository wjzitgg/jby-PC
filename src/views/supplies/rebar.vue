  ...
  <div class="app-container">
    ...
    <div class="search-bgc search-mr">
      <el-form :model="searchData" class="form-data">
        ...
        <el-form-item :label="user.orgType==5?'单据编号':'申请单号'" label-width="90px" v-if="typeShow">
          <el-input v-model="searchData.orderCode" size="mini" maxlength="50"></el-input>
        </el-form-item>
        <el-button  v-if="typeShow" size="mini" type="primary" class="seek" icon="el-icon-search" @click="queryData">查询</el-button>
        <el-button  v-if="typeShow" type="primary" size="mini" class="add-staff" @click="addRecord" v-has="'supplies:rebar:add'">新增采购计划单</el-button>
      </el-form>
    </div>
    <div class="tab-container" v-if="typeShow">
      <div class="table-data" style="height: 75%">
        <el-table :data="tableData" border v-loading="loading" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }" ref="tableBox" :tooltip-effect="'light'">
          ...
          <el-table-column prop="confirmUserName" label="确认人" align="center" v-if="user.orgType == 5||user.orgType == 4||user.orgType == 10"> </el-table-column>
          <el-table-column prop="confirmTime" label="确认时间" align="center" v-if="user.orgType == 5||user.orgType == 4||user.orgType == 10"> </el-table-column>
          <el-table-column label="状态" prop="purchaseCodeName" align="center"> </el-table-column>
          <el-table-column prop="remark" label="备注" align="center" show-overflow-tooltip> </el-table-column>
          <el-table-column label="操作" align="center" width="300">
            <template slot-scope="{ row }">
              <el-button size="mini" plain @click="lookOver(row, 1)" v-has="'supplies:rebar:query'">查看</el-button>
              <el-button size="mini" plain type="primary" @click="shareCode(row.pkId)" v-has="'supplies:rebar:viewCode'" v-show="row.isCode">分享二维码</el-button>
              <el-button size="mini" plain type="primary" v-has="'supplies:rebar:update'" @click="editRecord(row)" v-show="row.isUpdate">编辑</el-button>
              <el-button size="mini" plain type="danger" @click="deleteRecord2(row.pkId)" v-has="'supplies:rebar:withdraw'" v-show="row.isWithdraw">撤回</el-button>
              <el-button size="mini" plain type="danger" v-has="'supplies:rebar:delete'" @click="deleteRecord(row.pkId)" v-show="row.isDelete">删除</el-button>
            </template>
          </el-table-column>
            ...
            <el-row :span="24" class="el-row-data">
              ...
              <el-col :span="8">
                <el-form-item label="供应商" prop="fkCustomerId" label-width="110px">
                  <el-select size="mini" @change="cutSupplier" v-model="form.fkCustomerId" class="wh" :disabled="num != 0">
    ...
    <materialsSelect :materDialog.sync="materDialog" :selectMater="accessoryList" @getSelect="getSelect" :showType="2" :fkCustomId="form.fkCustomerId" materialTypeCode="materials_rebar" v-if="num != 1"></materialsSelect>
...
export default {
  ...
  data() {
    return {
      ...
      purchaseTypeCode: "purchase_rebar",
      // 搜索数据
      searchData: {
        projectName: "",
        purchaseCode: "",
  ...
  methods: {
    ...
    // 获取供应商
    supplier(id) {
      this.$api.searchNoLinkOrgId({ projectBidId: id, customType: 3, supplierType: 3 }).then(res => {
        if (res.code == 200) {
          res.data.forEach(item => {
    ...
    // 获取 关联物资申请单
    getApplicationData(query, fkSupplyCustomerId) {
    ...
    addRecord() {
      ...
      this.accessoryList = [{ fkMaterialId: 1, materialName: "", materialTypeName: "", unitName: "", purchaseNum: "" }]; // 初始化物料
      // 获取申请单号
      this.$api.getBusinessCode({ businessType: 6 }).then(res => {
        if (res.code === 200) {
    ...
    supCusChange(val) {
      // this.form.rePurchaseApplies = [];
      let fkSupplyCustomerId = val;
      this.getApplicationData("", fkSupplyCustomerId);
    },
    searchNoLink(arr) {
      let ids = [];
      arr.forEach(item => {
      ...
      this.$api.searchNoLink(data).then(res => {
        if (res.code == 200) {
          ...
          if (this.form.fkSupplyCustomerId != undefined && this.form.fkSupplyCustomerId != null && this.form.fkSupplyCustomerId != "") {
            let status = true;
            this.subcontractorsArr.forEach(item => {
              if (this.form.fkSupplyCustomerId == item.pkId) {
                status = false;
              }
            });
            this.form.fkSupplyCustomerId = status ? "" : this.form.fkSupplyCustomerId;
          }
        } else {
          this.$message.warning(res.msg);
        }
