<template>
  <div class="app-container">
    <!-- 头部搜索 -->
    <el-tabs v-model="topRadio" v-if="user.orgType === 5 || user.orgType === 4 || user.orgType === 10 && typeShow" @tab-click="typeChange">
    ...
    <div class="search-bgc search-mr">
      <el-form :model="searchData" class="form-data">
        ...
        <el-form-item label="标段项目" label-width="80px" v-if="user.orgType == 7 || (user.orgType == 6 && typeShow)">
          <el-select v-model="searchData.fkBidProjectId" size="mini" @change="projectIdChange">
            <el-option label="全部" value=""></el-option>
            <el-option v-for="item in noProjectData" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="分包单位" label-width="80px" v-if="(user.orgType == 5 || user.orgType == 4 || user.orgType == 10) && topRadio == 1 && typeShow">
          <el-select v-model="searchData.fkCustomerId" size="mini">
            <el-option label="全部" value=""></el-option>
            <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="供应商" label-width="80px" v-if="(user.orgType == 5 || user.orgType == 4 || user.orgType == 10) && topRadio == 2 && typeShow">
          <el-select v-model="searchData.fkCustomerId" size="mini">
            <el-option label="全部" value=""></el-option>
        ...
        <el-form-item class="writeTime" label="单据时间" label-width="90px" v-if="typeShow">
        ...
        <el-button type="primary" size="mini" class="add-staff" @click="addRecord" v-has="'supplies:request:add'" v-if="user.orgType != 6 && typeShow">新增申请单</el-button>
        <el-button type="primary" size="mini" class="add-staff" @click="addRecord2" v-has="'supplies:request:add'" v-if="user.orgType == 6 && typeShow">新增物资申请单</el-button>
      </el-form>
    </div>
    <div class="tab-container" v-loading="loading" v-if="typeShow">
        ...
        <el-table :tooltip-effect="'light'" :data="tableData" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }" ref="tableBox">
          ...
          <el-table-column label="操作" align="center" width="300">
            <template slot-scope="{ row }">
              <el-button size="mini" plain @click="lookOver(row, 1)" v-has="'supplies:request:query'">查看</el-button>
              <el-button size="mini" plain v-has="'supplies:request:update'" type="primary" @click="editRecord(row)" v-show="row.isUpdate">编辑</el-button>
              <el-button size="mini" plain v-has="'supplies:request:handle'" type="success" @click="lookOver(row, 3)" v-show="row.isHandle">处理</el-button>
              <el-button size="mini" plain v-has="'supplies:request:withdraw'" type="danger" @click="deleteRecord2(row.pkId)" v-show="row.isWithdraw">撤回</el-button>
              <el-button size="mini" plain v-has="'supplies:request:delete'" type="danger" @click="deleteRecord(row.pkId)" v-show="row.isDelete">删除</el-button>
            </template>
          </el-table-column>
      ...
      <div class="table-data" style="height: 72%" v-if="topRadio == 2">
        <el-table :tooltip-effect="'light'" :data="tableData" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }" ref="tableBox">
          ...
          <el-table-column label="操作" align="center" width="300">
            <template slot-scope="{ row }">
              <el-button size="mini" plain v-has="'supplies:request:query'" @click="lookOver2(row, 1)">查看</el-button>
              <el-button size="mini" plain v-has="'supplies:request:update'" type="primary" @click="editRecord2(row)" v-show="row.editStatus">编辑</el-button>
              <el-button size="mini" plain v-has="'supplies:request:handle'" type="success" @click="lookOver2(row, 3)" v-show="row.handleStatus">处理</el-button>
              <el-button size="mini" plain v-has="'supplies:request:withdraw'" type="danger" @click="deleteApply2(row.pkId)" v-show="row.cancelStatus">撤回</el-button>
              <el-button size="mini" plain v-has="'supplies:request:delete'" type="danger" @click="deleteApply(row.pkId)" v-show="row.deleteStatus">删除</el-button>
            </template>
          </el-table-column>
          <div slot="empty" class="empty" style="width: 100%">
            <img src="@/assets/empty.svg" class="zanwu" />
  ...
  methods: {
    selectIds(arr, obj, isTrue) {
      if (arr.length) {
        ...
        if (isTrue) {
          // this.supplier2(this.searchData.fkBidProjectId)
          this.queryData();
        }
    ...
    // 获取供应商
    supplier2(id) {
      this.$api.searchByProjectIds({ customType: 3, projectId: id }).then(res => {
        if (res.code == 200) {
          res.data.forEach(item => {
            item.label = item.customName;
            item.value = item.pkId;
    ...
    addRecord2() {
      ...
      this.title2 = "新增物资申请单";
      this.popup2 = true;
      this.form.issueCode = this.outObjectList[0].keyName;
      this.fkPurchaseId = {};
      this.random = Math.random();
      this.$nextTick(() => {
