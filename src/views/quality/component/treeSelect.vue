<template>
  <div style="position: relative;">
    <div @click="changeTreeShow" style="position: relative;">
      <el-input class="input" size="mini" :disabled="disabled" :value="treeDateItem ? treeDateItem.itemName : proIds.itemName" clearable @clear="clearable">
    ...
    <div class="treeBox" v-show="showTree">
      <el-input v-model="filterText" size="mini" clearable class="text-select" v-if="!showCheckbox && treeList.length > 0" @focus.stop="openTree">
        <i slot="prefix" class="el-input__icon el-icon-search"></i>
      </el-input>
      <el-tree @node-expand="nodeExpand" :data="treeList" :check-strictly="true" @check="checkChange" default-expand-all :show-checkbox="showCheckbox" :props="defaultProps" :expand-on-click-node="false" ref="tree" :node-key="nodeKey" :filter-node-method="filterNode" v-if="showCheckbox">
        <div slot-scope="{ node }" :title="node.label" class="tree-items">
          {{ node.label }}
        </div>
      </el-tree>
      <el-tree @node-expand="nodeExpand" :data="treeList" :check-strictly="true" @check="checkChange" default-expand-all :show-checkbox="showCheckbox" :props="defaultProps" :expand-on-click-node="false" ref="tree" :node-key="nodeKey" :filter-node-method="filterNode" @node-click="nodeClick" v-else>
        <div slot-scope="{ node }" :title="node.label" class="tree-items">
          {{ node.label }}
        </div>
      </el-tree>
...
export default {
  ...
  methods: {
    ...
    setTreeSelect(ids) {
      ...
      this.proIds = this.$refs.tree.getCheckedNodes(true)[0];
    },
    ...
    filterNode(value, data, node) {
      ...
      function getLevel(data) {
        console.log(data);
        if (data.level !== 0) {
          result[data.level] = data.data.itemName.indexOf(value) !== -1;
          getLevel(data.parent);
        }
      }
      getLevel(node);
