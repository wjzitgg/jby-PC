  ...
  <div class="app-container">
    ...
    <div class="search-bgc search-mr">
      <el-form :model="searchData" class="form-data search-bar-box">
        ...
        <el-button v-if="typeShow" size="mini" type="primary" class="seek" @click="queryData" icon="el-icon-search">查询</el-button>
        <el-button v-if="typeShow" type="primary" size="mini" class="add-staff" @click="addRecord" v-has="'quality:management:add'">新增记录</el-button>
      </el-form>
    </div>
      ...
      <el-table :data="tableData" v-loading="loading" height="100%" :header-cell-style="{ background: '#f1f6ff' }" :tooltip-effect="'light'">
        ...
        <el-table-column label="操作" align="center" width="400">
          <template slot-scope="{ row }">
            <el-button plain size="mini" type="primary" @click.native="lookOver(row)" v-has="'quality:management:query'">查看</el-button>
            <el-button plain size="mini" type="success" v-has="'quality:management:update'" @click.native="editRecord(row)" v-if="row.editStatus">编辑</el-button>
            <el-button plain size="mini" @click.native="handleDownload(row.enclosures)" v-if="row.enclosures && row.enclosures.length" v-has="'quality:management:download'">下载</el-button>
            <el-button plain size="mini" type="danger" v-has="'quality:management:delete'" @click.native="deleteRecord(row.pkId)" v-if="row.deleteStatus">删除</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog :close-on-press-escape="false" :title="title" :visible.sync="popup" v-dialogDrag :close-on-click-modal="false" v-loading="loading" @close="close(false)" class="tree-list">
      <div class="dialog-form">
        <el-form :model="form" :rules="rules" ref="form">
          <el-form-item label="分项工程" prop="projectName" label-width="90px">
          ...
          <el-form-item label="记录内容" label-width="90px" prop="remark">
          ...
          <el-form-item label="附件" label-width="90px" prop="accessory">
            <div>
              <el-button size="small" type="primary" @click.native="uploading" v-if="form.accessory.length != 10 && !disabledStatus" :disabled="disabledStatus">点击上传</el-button>
              ...
              <div v-if="form.accessory.length == 0 && disabledStatus">无附件</div>
            </div>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    addRecord() {
      ...
      this.title = "新增记录";
      console.log(this.form.accessory);
      this.popup = true;
      this.$nextTick(() => {
...
<style lang="scss">
/deep/ .writeTime .el-form-item__content {
  display: flex;
}
</style>
