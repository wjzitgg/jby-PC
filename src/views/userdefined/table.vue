  ...
  <div class="app-container">
    <div class="search-bgc search-mr">
      <el-form :model="searchData" class="form-data">
        ...
        <el-form-item label="专业类别" label-width="80px" v-if="user.orgType == 5 || user.orgType == 0">
        ...
        <el-button size="mini" type="primary" icon="el-icon-search" class="seek" @click="search(searchData)">搜索</el-button>
        <el-button type="primary" size="mini" v-has="'userdefined:table:add'" class="add-staff btn-right" @click="addUnit">新建表格</el-button>
      </el-form>
    </div>
    <div class="tab-container">
      <div class="table-data" style="height: 83%">
        <el-table :data="tableData" stripe ref="tables" height="95%" style="width: 100%" v-loading="loading" :tooltip-effect="'light'" :header-cell-style="{ background: '#f1f6ff' }" :cell-style="{ align: 'center' }">
          ...
          <!-- <el-table-column align="center" v-if="user.orgType == 5" prop="enginerringTypeName" label="工程大类" min-width="120"></el-table-column> -->
          <el-table-column align="center" v-if="user.orgType == 5" prop="professionalTypeName" label="专业类别" min-width="120"></el-table-column>
          <el-table-column align="center" prop="tableName" label="表格名称" min-width="120"></el-table-column>
          <el-table-column align="center" prop="tableType" label="表格类型" min-width="120" v-if="user.orgType == 5">
            <template slot-scope="scope">
              <span v-if="scope.row.tableType == 0">普通表格</span>
              <span v-if="scope.row.tableType == 1">质量评定表</span>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="sortval" label="排序值" min-width="60"></el-table-column>
          ...
          <el-table-column align="center" prop="alarmLevel" label="操作" min-width="220">
            ...
            <el-button type="primary" plain size="mini" v-has="'system:form:query'"
              ...
              <el-button type="primary" plain size="mini" v-has="'userdefined:table:query'" @click.native="examine(scope.row)">预览</el-button>
              <el-button size="mini" plain @click.native="edit(scope.row)" v-has="'userdefined:table:update'">编辑</el-button>
              <el-button type="danger" plain size="mini" v-has="'userdefined:table:delete'" @click.native="remove(scope.row)">删除</el-button>
              <el-button type="primary" plain size="mini" @click.native="copyAdd(scope.row)">复制新增</el-button>
            </template>
          </el-table-column>
    ...
    <el-dialog :title="addTitle" :visible.sync="show1" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <div style="position: relative" class="dialog-form">
        <el-form :model="form" :rules="rules" ref="form">
          ...
          <el-form-item label="专业类别" v-if="user.orgType == 5 || user.orgType == 0" prop="fkProfessionalTypeId" :label-width="formLabelWidth">
          ...
          <el-form-item label="表格类型" prop="tableType" :label-width="formLabelWidth" v-if="user.orgType == 5">
            <el-select style="width: 100%" :disabled="user.orgType != 5 && user.orgType != 0" v-model="form.tableType" placeholder="请选择">
          ...
          <el-form-item label="表格名称" prop="tableName" :label-width="formLabelWidth">
            <el-input v-model="form.tableName" autocomplete="off" :disabled="showType == 4" maxlength="50"></el-input>
          </el-form-item>
          <el-form-item label="排序值" prop="sortval" :label-width="formLabelWidth">
            <el-input v-model="form.sortval" autocomplete="off" type="number" oninput="if(value.length > 5) value=value.slice(0, 5)" @keydown.native="channelInputLimit" :disabled="showType == 4"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    submit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          // return console.log(this.form.fkProfessionalTypeId)
          this.loading = true;
          if (this.showType == 1) {
            const addForm = { ...this.form };
            ...
            // }
            addForm.fkEnginerringTypeId = this.user.fkEnginerringTypeId;
    ...
    edit(row) {
      this.$api.queryForm({ pkId: row.pkId }).then(res => {
        if (res.code == 200) {
          ...
          this.form.remark = rows.remark;
          this.form.fkEnginerringTypeId = rows.fkEnginerringTypeId;
          this.form.fkProfessionalTypeId = rows.fkProfessionalTypeId == 0 ? "0" : rows.fkProfessionalTypeId;
          this.form.tableHtml = JSON.stringify(utils.unzip(rows.tableHtml));
          this.form.tableHead = rows.tableHead;
          this.form.tableData = JSON.stringify(utils.unzip(rows.tableData));
          this.form.tableType = rows.tableType + "";
      ...
      this.$api.queryForm({ pkId: row.pkId }).then(res => {
        if (res.code == 200) {
          ...
          this.form = {
            ...
            tableHtml: utils.unzip(rows.tableHtml), // 表格内容
            fkEnginerringTypeId: res.data.fkEnginerringTypeId,
            fkProfessionalTypeId: res.data.fkProfessionalTypeId,
