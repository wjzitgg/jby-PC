  ...
  <div class="app-container">
    ...
    <div class="search-bgc search-mr">
      <el-form :model="searchData" class="form-data">
        ...
        <el-form-item label="规则名称" label-width="90px">
          <el-input v-model="searchData.ruleName" size="mini" class="wh-200" maxlength="25"></el-input>
        </el-form-item>
        <el-button size="mini" type="primary" icon="el-icon-search" class="seek" @click="queryData">搜索</el-button>
        <el-button type="primary" size="mini" v-has="'userdefined:order:add'" class="add-staff btn-right" @click="addRecord">新增规则</el-button>
      </el-form>
    </div>
    <div class="tab-container">
      <div class="table-data" style="height:91%;margin-top: 10px;">
        <el-table :data="tableData" stripe v-loading="loading" height="90%" :header-cell-style="{ background: '#f1f6ff' }">
          ...
          <el-table-column label="操作" align="center" width="400">
            <template slot-scope="{ row }">
              <el-button size="mini" plain type="primary" v-has="'userdefined:order:query'" @click="examine(row.pkId)">查看</el-button>
              <el-button size="mini" plain type="warning" v-has="'userdefined:order:update'" @click="editOrder(row.pkId)">编辑</el-button>
              <el-button size="mini" plain type="success" v-has="'userdefined:order:configure'" @click="configuration(row.pkId)">配置单据</el-button>
              <el-button size="mini" plain type="danger" v-has="'userdefined:order:delete'" @click="deleteOrder(row.pkId)">删除</el-button>
            </template>
          </el-table-column>
    ...
    <el-dialog title="配置单据" width="400px" :visible.sync="configurationShow" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <div class="dialog-form">
        <div class="rule">规则名称 <span class="weight-600"></span> &nbsp;&nbsp; &nbsp;<el-input disabled v-model="ruleData.ruleName" size="mini" class="wh-300"></el-input></div>
        <!-- <div class="rule" style="text-indent: 2em;">规则名称</div> -->

        <div class="ml-20">
          <el-checkbox-group v-model="checkList">
            <el-checkbox class="rule-check-item" v-for="(item, idx) in billsType" :key="idx" :label="item.key" :disabled="item.forbidden" :value="item.key">{{ item.name }}</el-checkbox>
          </el-checkbox-group>
        </div>
      </div>
      <div slot="footer">
        <el-button size="mini" @click="configurationShow = false">取 消</el-button>
        <el-button size="mini" type="primary" @click="ruleSubmit">保 存</el-button>
      </div>
    </el-dialog>
    <!-- 新增修改弹窗  -->
    <!-- class="dialog-form" -->
    <el-dialog :title="ruleTittle" width="600px" :visible.sync="ruleShow" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      <!-- <div class="dialog-form"> -->
      <el-form :model="ruleData" :rules="rules" ref="form" class="row-height dialog-form">
        <div>
          <el-row :span="24" class="el-row-data">
            <el-col :span="24">
              <el-form-item label="规则名称" prop="ruleName" label-width="110px">
                <el-input v-model="ruleData.ruleName" size="mini" maxlength="25"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="14">
              <el-form-item label="允许手动修改" prop="allowModify" label-width="110px">
            ...
            <el-col :span="10" v-if="ruleData.allowModify == '0'">
              <el-form-item label="限制长度" prop="limitNum" label-width="100px">
                <!-- ruleData.limitNum.replace(/^(0+)|[^\d]+/g, '') -->
                <el-input v-model="ruleData.limitNum" maxlength="2" max="50" @input="limitInput" size="mini"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <div class="rule">
          ...
          <div v-for="(item, idx) in ruleDetails" :key="idx" class="rule-numerical">
            ...
            <el-select v-model="item.ruleCode" size="mini" class="wh-200" @change="ruleCodeChange(idx)">
              <el-option v-for="(item, idx) in getList(idx)" :key="idx" :label="item.keyVal" :value="item.keyName"></el-option>
            </el-select>
            <el-input maxlength="9" v-if="item.ruleCode == 1" v-model="item.ruleHead" size="mini" class="wh-222"></el-input>

            <el-input v-if="item.ruleCode == 3" @input="item.ruleHead = item.ruleHead.replace(/^(0+)|[^\d]+/g, '')" v-model="item.ruleHead" size="mini" maxlength="1" class="wh-203"></el-input>
            <span v-if="item.ruleCode == 3">位</span>
            <el-input v-if="item.ruleCode == 4 || item.ruleCode == 5 || item.ruleCode == 6 || item.ruleCode == 7" @input="item.ruleHead = item.ruleHead.replace(/^(0+)|[^\d]+/g, '')" v-model="item.ruleHead" size="mini" maxlength="1" class="wh-90"></el-input>
            <span v-if="item.ruleCode == 4 || item.ruleCode == 5 || item.ruleCode == 6 || item.ruleCode == 7">至</span>
            <el-input v-if="item.ruleCode == 4 || item.ruleCode == 5 || item.ruleCode == 6 || item.ruleCode == 7" @input="item.ruleTail = item.ruleTail.replace(/^(0+)|[^\d]+/g, '')" v-model="item.ruleTail" size="mini" maxlength="1" class="wh-90"></el-input>
            ...
            <el-button v-if="item.ruleCode != 8" @click="addDetails(idx)" class="border-no color-green" icon="  el-icon-plus"></el-button>
          </div>
        </div>
      </el-form>
      <div slot="footer">
        <el-button size="mini" @click="ruleShow = false">取 消</el-button>
        <el-button size="mini" type="primary" @click="submitRule('form')">保 存</el-button>
      </div>
    </el-dialog>
    <!-- 查看弹窗 -->
    <el-dialog title="单号规则信息" width="600px" :visible.sync="examineShow" v-dialogDrag :close-on-click-modal="false" :close-on-press-escape="false" v-loading="loading">
      ...
      <div class="dialog-form">
        <div v-show="tabPosition == 1" class="examineShow-box">
          <div class="rule" style="text-indent: 0em;">规则名称 <el-input disabled v-model="ruleData.ruleName" size="mini" class="wh-300"></el-input></div>
          <div class="rule">
            允许手动修改
            <el-radio-group class="rule-ruleData" v-model="ruleData.allowModify">
...
export default {
  ...
  methods: {
    limitInput(e) {
      this.ruleData.limitNum = this.$limitInputNumber(e, 50);
    },
    getList(idx) {
      return idx == 0 ? this.ruleList : this.ruleList.filter(item => item.keyName != 8);
    },
    ruleCodeChange(idx) {
      if (idx == 0 && this.ruleDetails[idx].ruleCode == 8) {
        this.ruleDetails = [this.ruleDetails[0]];
      }
      this.ruleDetails[idx].ruleHead = "";
      this.ruleDetails[idx].ruleTail = "";
    },
    searchOrderTypeByOrgId() {
      this.$api.searchOrderTypeByOrgId().then(res => {
        if (res.code == 200) {
          this.billsType = res.data;
          this.billsType.forEach(item => {
    ...
    configuration(pkId) {
      ...
      this.loading = true;
      this.$api.findByIdNoJurisdiction({ pkId }).then(res => {
        this.loading = false;
        if (res.code == 200) {
          this.ruleData.ruleName = res.data.ruleName;
          this.checkList = res.data.customOrderSets;
          ...
          this.billsType.forEach(item => {
            if (res.data.customOrderSetDisables.length != 0) {
              res.data.customOrderSetDisables.forEach(e => {
                if (item.key == e) {
                  item.forbidden = true;
                }
              });
          ...
          this.billsType.forEach(item => {
            if (res.data.customOrderSetDisables.length != 0) {
              res.data.customOrderSets.forEach(e => {
                if (item.key == e) {
                  item.forbidden = false;
                }
              });
    ...
    addDetails(idx) {
      ...
      let item = { ruleCode: "", ruleHead: "", ruleTail: "" };
      this.ruleDetails.splice(idx + 1, 0, item);
    },
    // 移除编码规则
    removeDetails(idx) {
    ...
    submitRule(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          let status = false;
          let num = 0;
          console.log(data.customOrderRuleSets);
          data.customOrderRuleSets.forEach((item, idx) => {
            item.sortval = idx;
            if (item.ruleCode == "" || item.ruleHead == "") {
              status = true;
            }
            if (item.ruleCode > 3) {
              num = num + Number(item.ruleTail);
            } else if (item.ruleCode == 3) {
              num = num + Number(item.ruleHead);
            } else {
              num = num + item.ruleHead.length;
            }
          });
          console.log(num);
          if (this.ruleData.allowModify == 0 && num > this.ruleData.limitNum) {
            return this.$message.warning("单据生成规则长度不能超过设置限制长度");
          }
          if (this.ruleData.allowModify == 1 && num > 50) {
            return this.$message.warning("单据生成规则长度不能大于50位");
          }

          if (status) {
            return this.$message.error("单据生产规则不能为空");
          }
