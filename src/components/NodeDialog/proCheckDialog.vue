  ...
  <div>
    ...
    <el-dialog :close-on-press-escape="false" title="分项工程审批流程详情" top="5vh" :visible="proChangeDialog" @close="btnClose" width="70%" v-dialogDrag :close-on-click-modal="false" v-loading="loading">
      <div class="dialog-form">
        <ul class="aside">
          <li class="aside-li wh100" :class="{wh75:data.checkResult&&data.checkResult!==0}">
          ...
          <li class="aside-li wh25" v-if="data.checkResult&&data.checkResult!==0">
            ...
            <div class="aside-div-right">
              <el-tooltip effect="dark" placement="bottom-start">
                <span>{{ ["不合格", "合格", "良好"][data.checkResult] }}</span>
              </el-tooltip>
            </div>
          </li>
          <li class="aside-li wh25">
            ...
            <div class="aside-div-right">
              <el-tooltip effect="dark" :content="basicInfo.unitProject" placement="bottom-start">
                <span>{{ basicInfo.unitProject }}</span>
              </el-tooltip>
            </div>
          </li>
          <li class="aside-li wh25">
            ...
            <div class="aside-div-right">
              <el-tooltip effect="dark" :content="basicInfo.partProject" placement="bottom-start">
                <span>{{ basicInfo.partProject }}</span>
              </el-tooltip>
            </div>
          </li>
          <li class="aside-li wh50">
            ...
            <div class="aside-div-right">
              <el-tooltip effect="dark" :content="basicInfo.outProject" placement="bottom-start">
                <span>{{ basicInfo.outProject }}</span>
              </el-tooltip>
            </div>
          </li>
          ...
          <li class="aside-li wh25" v-if="!startNode">
          ...
          <li class="aside-li wh25" v-if="startNode">
            ...
            <div class="aside-div-right">
              {{ itemData.approveComment }}
            </div>
          </li>
          <li class="aside-li wh25" v-if="startNode">
            <div class="aside-div-left">发起人</div>
            <div class="aside-div-right">
              {{ itemData.approver }}
            </div>
          </li>
          <li class="aside-li wh25" v-if="startNode">
            <div class="aside-div-left">发起时间</div>
            <div class="aside-div-right">
              {{ itemData.approveTime }}
            </div>
          </li>
        </ul>
          ...
          <div :class="{ 'right-content': [4, 5, 6].includes(appDiaType) }" :style="{ width: [4, 5, 6].includes(appDiaType) ? '' : '100%' }">
            ...
            <div v-show="nowTab === 1" v-if="appDiaType != 6">
              <el-tabs v-model="activeName" type="card" class="box-dialog">
                <el-tab-pane label="审批流程" name="basicInform">
                  <!-- <el-container> -->
                  <!-- 左侧 -->
                  ...
                  <el-main class="dialogMain2">
                    ...
                    <taskFinishList :list="list" :show="!!data.isApprovalFailedStatus" :current-node-id="data.currentNodeId"></taskFinishList>
                  </el-main>
                  <!-- </el-container> -->
                </el-tab-pane>
                ...
                <el-tab-pane :label="item.tableName" v-for="(item, index) in linkTableLift" :key="index" :name="index + 'table'">
                  <el-container>
                    <!-- 左侧 -->
                    <!-- <el-aside class="lf-aside">
                      <div class="tables table-data">
                        <el-table :tooltip-effect="'light'" :data="linkTableLift" height="100%" stripe :header-cell-style="{ background: '#f1f6ff' }">
                            ...
                            <template slot-scope="scope">
                              <el-radio-group v-model="tableRadio" size="mini" @change="tabChange(scope.row, scope.$index)">
                                <el-tooltip class="item" effect="dark" :content="scope.row.tableName" placement="top" :disabled="scope.row.tableName.length > 4 ? false : true">
                                  <el-radio-button :label="scope.row.fkTableId" v-if="scope.row.tableName">
                                    {{ scope.row.tableName }}
                                  </el-radio-button>
                    ...
                    <el-main class="dialogMain">
                      <div class="html-content">
                        <table style="border-collapse: collapse;overflow: auto;margin: 0 auto;" ref="printPage" v-if="linkTableLift.length != 0">
    ...
    <approveCodeDialog ref="approveCodeDialog" v-if="proChangeDialog" :approveCodeDialog.sync="approveCodeDialog" @closeLoading="loading = false" :sign-date="signDate" @approveOk="withdraw" :noApprove="true" :biz-type="0" />
  </div>
</template>
...
export default {
  components: { pdfPiev, taskFinishList, chapterProcessApproval, signPdf, NodeDialog2, approveCodeDialog },
  props: {
    proChangeDialog: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    withdraw(unique) {
      if (this.appDiaType === 3) {
        this.$api
          .withdrawFlow({ approveId: this.data.pkId,unique:unique })
          .then(res => {
            if (res.code === 200) {
        ...
        this.$api.revocationWithdrawSeal({ pkId: this.data.pkId }).then(res => {
          if (res.code == 200) {
            this.$message.success("撤回成功");
            this.$emit("update:proChangeDialog", false);
            this.$emit("getList");
    ...
    urlPush(url, type) {
      const width = this.pdfTitle[this.pdfUrlData.length].type == "2" ? 842 : 596;
      const height = this.pdfTitle[this.pdfUrlData.length].type == "2" ? 596 : 842;
      this.pdfUrlData.push({ path: url, width, height, name: this.pdfTitle[this.pdfUrlData.length].title, direction: this.pdfTitle[this.pdfUrlData.length].type });
  ...
  watch: {
    proChangeDialog(val) {
      if (val) {
        ...
        this.linkTableLift = this.data.workflowTableList; // 关联表格
        this.workAmount = this.data.prodItemList; // 工程量
        this.materials = this.data.materialItemList; // 材料用量
        ...
        console.log(this.linkTableLift);
        if (this.linkTableLift && this.linkTableLift.length) {
          this.tableRadio = this.linkTableLift[0].fkTableId;
          this.tabTitle = this.linkTableLift[0].tableName;
          this.tabContent = this.linkTableLift[0].tableHtml;
          return;
