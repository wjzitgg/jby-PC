<template>
  <div class="picture-upload-comp" :class="photoClass">
    <div class="upload-bd">
      <div :key="index" class="el-upload-list" v-for="(file, index) in fileList">
        <img :src="file" alt class="el-upload-list__item-thumbnail" style="height:100%" />
        <span class="el-upload-list__item-actions">
          <span @click="handlePictureCardPreview(file)" class="el-upload-list__item-preview">
            <i class="el-icon-zoom-in"></i>
          </span>
          <span @click="handleRemove(file)" class="el-upload-delete" v-if="showUpload">
            <i class="el-icon-delete"></i>
          </span>
        </span>
      </div>
      <el-upload style="width:100%" :before-upload="beforeUpload" :accept="accept" :http-request="uploadFile" :limit="limit" :multiple="multiple || false" :on-error="uploadError" :on-remove="handleRemove" :show-file-list="false" action list-type="picture-card" ref="upload" v-if="fileList.length == 0">
    ...
    <el-dialog :visible.sync="dialogVisible" :modal="false" :title="viewTitle || '预览图片'" top="5vw">
      <div class="preview">
...
export default {
  ...
  methods: {
    ...
    async uploadFile(files) {
      ...
      if (self.accept && self.accept.indexOf(files.file.type.split("/").reverse()[0]) < 0) {
      ...
      if (this.imgStatus == true) {
        ...
        const blobFile = this.dataURLtoBlob(files.file);
        ...
        this.$api
          ...
          .then(res => {
            if (res.code == 200) {
              console.log(res);
              const sign = res.data.token;
              const path = res.data.endpoint;
              this.uploadDate = res.data;
              this.upLoads(sign, path, blobFile, filename, res.data.tmpSecretKey, res.data.tmpSecretId, res.data.startTime, res.data.expiredTime);
            } else {
              this.uploadError();
            }
          })
