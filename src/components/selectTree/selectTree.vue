  ...
  <div style="position: relative;">
    <div @click="changeTreeShow" style="position: relative;">
      ...
      <div class="select-tags">
        <el-tag class="sel-tag" size="mini" closable type="info" @close="closeTag" v-for="item in proIds" :key="item.pkId">
          {{ item.itemName }}
        </el-tag>
    ...
    <div class="treeBox" v-show="showTree">
      <el-tree :data="treeList" :check-strictly="true" @node-expand="nodeExpand" @check-change="checkChange" default-expand-all show-checkbox :props="defaultProps" :expand-on-click-node="false" ref="tree" :node-key="nodeKey">
...
export default {
    ...
    checkedList: {
      ...
      handler(val) {
        if (val.length) {
          ...
          this.proIds = this.$refs.tree.getCheckedNodes();
        }
      }
    }
  },
  ...
  methods: {
    ...
    // 每次勾选取消都会执行
    checkChange(obj, type) {
      console.log(type, obj.grade);
      const arr = this.$refs.tree.getCheckedKeys();
      // 父级选中的时候
      if (type == true && obj.grade < 3) {
        obj.children.forEach(item => {
          // 判断选中状态数组里，在children是否存在，清空选中的对应pkid
          if (arr.findIndex(z => z == item.pkId) !== -1) {
            arr.splice(
              arr.findIndex(z => z == item.pkId),
              1
            );
          }
          ...
          if (item.children) {
            item.children.forEach(i => {
              if (arr.findIndex(j => j == i.pkId) !== -1) {
                arr.splice(
                  arr.findIndex(j => j == i.pkId),
                  1
                );
              }
      ...
      // 取消选中最子集的bug问题
      if (type == false && obj.grade == 2 && obj.disabled) {
        obj.children.forEach(item => {
          this.$set(item, "disabled", true);
        });
      }
      // console.log("计算后的数组", arr);
