<template>
  <div class="panelWrapper">
    <div class="panelHeader">节点信息</div>
    <el-form class="panelForm" :model="form" label-position="left">
      ...
      <div v-if="businessObjectType == 'bpmn:UserTask'">
        ...
        <div class="panleCenten" v-if="workflowType.length">
          <el-radio-group :disabled="forbidden" v-model="form.nodeApproveType" @change="v => updateForm('nodeApproveType', v)">
            <el-radio :label="'0'" v-if="workflowType == '8'">审批节点</el-radio>
          ...
          <div v-if="form.nodeApproveType != 3">
            ...
            <el-row>
              ...
              <el-select :disabled="forbidden" v-model="form.unitId" placeholder="请选择" size="mini" @change="v => unitOptionsChang('unitId', v)">
                <el-option v-for="item in unitOptions" :key="item.pkId" :label="item.roleName" :value="item.pkId"> </el-option>
              </el-select>
            </el-col>
            <el-col :span="12">
              <el-select :disabled="forbidden" v-model="form.fkRoleId" placeholder="请选择" size="mini" @change="v => updateForm('fkRoleId', v)">
          ...
          <div v-if="form.nodeApproveType == 3">
            ...
            <el-col :span="12">
              <el-select :disabled="forbidden" v-model="form.fkUserId" placeholder="请选择" size="mini" @change="v => unitOptionsChang('fkUserId', v)">
                <el-option v-for="item in chapterList" :key="item.pkId" :label="item.userName" :value="item.pkId"> </el-option>
              </el-select>
            </el-col>
          </el-row>
          </div>
      ...
      <div v-if="businessObjectType == 'bpmn:UserTask' && workflowType != ''" >
        <div class="panleTitle"><i class="el-icon-s-order"></i> 动态表格设计</div>
        <div class="panleCenten" v-if=" workflowType != '8'">
          <el-radio-group :disabled="forbidden" v-model="form.tableSource" @change="v => tableSourceForm('tableSource', v)">
        ...
        <div class="table-pull-box" v-if="checkStatus" :class="{'table-max-height':workflowType==0}">
          <el-row v-for="(item, index) in tableIdForm" :key="index" style="margin: 4px 0px;">
            ...
            <el-col :span="7" style="line-height: 22px;padding-left: 2px;">
              <el-checkbox :disabled="forbidden" v-model="item.archives" v-if="user.orgType == 5" @change="v => archivesChang(v)">档案表</el-checkbox>
            </el-col>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.businessObjectType = this.businessObject.$type;
      this.form = this.businessObject;
      this.form.fkUserId = this.businessObject.$attrs.fkUserId;
      this.form.tableSource = this.businessObject.$attrs.tableSource;
      ...
      if (this.businessObject.$attrs.archives != undefined && this.businessObject.$attrs.archives != null && this.businessObject.$attrs.archives.length && typeof this.businessObject.$attrs.archives == "string") {
        ...
        if(this.businessObject.$attrs.archives!=undefined && this.businessObject.$attrs.archives.length){
          this.form.archives.forEach((item, idx) => {
          if (item == '' || item == 0) {
            item = false
            this.tableIdForm[idx].archives = false
...
.table-pull-box{
  border-top: 1px solid;height: 130px;overflow: auto;padding: 5px;font-size: 11px;
}
.table-max-height{
  height: 100px;
}
