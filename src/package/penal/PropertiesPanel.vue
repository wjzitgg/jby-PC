<template>
  <div class="process-panel__container" :style="{ width: `${this.width}px` }">
    ...
    <el-collapse v-model="activeTab">
      <el-collapse-item name="base" v-show="elementType !='Process' ">
      ...
      <el-collapse-item name="base" v-if="elementType === 'SubProcess'" key="message">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-order"></i>子流程设置</div>
        <signal-and-massage :id="elementId" :type="elementType" :business-object="elementBusinessObject" />
      </el-collapse-item>
      <!-- <el-collapse-item name="condition" v-show="false" v-if="conditionFormVisible" key="condition">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-promotion"></i>流转条件</div>
        <flow-condition :business-object="elementBusinessObject" :type="elementType" />
      </el-collapse-item> -->
      <el-collapse-item name="base" v-if="elementType ==='UserTask'" key="form">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-help"></i>节点设置</div>
        <element-form :id="elementId" :type="elementType" />
      </el-collapse-item>
      <!-- <el-collapse-item name="task" v-show="false" v-if="elementType.indexOf('Task') !== -1" key="task">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-claim"></i>任务</div>
        <element-task :id="elementId" :type="elementType" />
      </el-collapse-item> -->
      <!-- <el-collapse-item name="multiInstance"  v-show="false"  v-if="elementType.indexOf('Task') !== -1" key="multiInstance">
        <div slot="title" class="panel-tab__title"><i class="el-icon-s-help"></i>多实例</div>
        <element-multi-instance :business-object="elementBusinessObject" :type="elementType" />
      </el-collapse-item> -->
      <!-- <el-collapse-item name="listeners"  v-show="false"  key="listeners">
        <div slot="title" class="panel-tab__title"><i class="el-icon-message-solid"></i>执行监听器</div>
        <element-listeners :id="elementId" :type="elementType" />
      </el-collapse-item> -->
      <!-- <el-collapse-item name="taskListeners"  v-show="false"  v-if="elementType === 'UserTask'" key="taskListeners">
        <div slot="title" class="panel-tab__title"><i class="el-icon-message-solid"></i>任务监听器</div>
        <user-task-listeners :id="elementId" :type="elementType" />
      </el-collapse-item> -->
      <!-- <el-collapse-item name="base"    key="extensions">
...
// import ElementProperties from "./signal-message/ElementForm";
